<?php

namespace App\Http\Controllers\Admin;

use Exception;
use Carbon\Carbon;
use App\Models\Team;
use App\Models\Player;
use App\Models\PlayerStat;
use App\Models\Tournament;
use App\Models\MatchPlayer;
use App\Models\Partnership;
use App\Models\CricketMatch;
use App\Models\FallOfWicket;
use Illuminate\Http\Request;
use App\Models\MatchDelivery;
use App\Models\MatchScoreBoard;
use App\Models\CricketMatchToss;
use App\Events\CricketMatchUpdateEvent;
use App\Models\TournamentTeamStat;
use Illuminate\Support\Facades\DB;
use App\Models\TournamentGroupTeam;
use Illuminate\Support\Facades\Log;
use App\Http\Controllers\Controller;
use App\Models\TournamentPlayerStat;
use Illuminate\Support\Facades\Auth;
use App\Events\CricketMatchTossEvent;
use Illuminate\Support\Facades\Validator;
use Illuminate\Database\Eloquent\ModelNotFoundException;

class CricketMatchController extends Controller
{
    protected $module = 'cricket-matches';

    protected function schemaHasColumn($table, $column)
    {
        try {
            return \Schema::hasColumn($table, $column);
        } catch (\Exception $e) {
            return false;
        }
    }

    protected function canAny(array $permissions): bool
    {
        $user = Auth::user();

        if (!$user) {
            return false;
        }

        foreach ($permissions as $permission) {
            if ($user->can($permission)) {
                return true;
            }
        }

        return false;
    }

    protected function canViewScoreboard(): bool
    {
        return $this->canAny([
            $this->module . '-scoreboard',
            'scoreboard-view',
            'scoreboard-edit',
            $this->module . '-start',
        ]);
    }

    protected function canManageScoreboard(): bool
    {
        return $this->canAny([
            $this->module . '-scoreboard',
            'scoreboard-edit',
        ]);
    }

    protected function unauthorizedJson()
    {
        return response()->json([
            'success' => false,
            'message' => 'Unauthorized Access',
        ], 403);
    }

    public function index()
    {
        try {
            if (Auth::user() && !Auth::user()->can($this->module . '-view')) {
                throw new Exception('Unauthorized Access');
            }

            session([
                'title' => 'Daily Cricket Matches',
                'breadcrumbs' => [
                    'home' => [
                        'url' => route('admin.dashboard'),
                        'name' => 'Dashboard'
                    ],
                    $this->module => [
                        'url' => route('admin.tournaments.index'),
                        'name' => 'Daily Cricket Matches Management'
                    ]
                ]
            ]);

            $today = Carbon::today();
            $validTeams = Team::all();
            $cricketMatchesList = CricketMatch::orderBy('created_at', 'desc')->get();
            return view('admin.pages.cricket-matches.index', compact('cricketMatchesList', 'validTeams'));
        } catch (Exception $e) {
            Log::error("Error Loading cricket-matches Management", [
                'line' => $e->getLine(),
                'file' => $e->getFile(),
                'message' => $e->getMessage()
            ]);

            return redirect()->back()->with([
                'success' => false,
                'message' => 'Error Loading cricket-matches Management.'
            ]);
        }
    }

    public function tableLoader(Request $request)
    {
        try {
            if (Auth::user() && !Auth::user()->can($this->module . '-view')) {
                throw new Exception('Unauthorized Access');
            }

            $matches = CricketMatch::with([
                'teamA',
                'teamB',
                'tournament',
                'winningTeam',
                'scoreboard' // relation to MatchScoreBoard
            ])
                ->orderBy('created_at', 'desc')
                ->get();

            $formattedData = $matches->map(function ($match) {

                $viewUrl = route('admin.cricket-matches.show', $match->id);
                $editUrl = route('admin.cricket-matches.edit', $match->id);
                $startUrl = route('admin.cricket-matches.scoreboard.view', $match->id);

                /* ===============================
                TEAM SCORES (LATEST INNINGS)
                ================================*/
                $teamAScore = null;
                $teamBScore = null;

                foreach ($match->scoreboard as $sb) {
                    $scoreText = "{$sb->runs}/{$sb->wickets} ({$sb->overs})";

                    if ($sb->team_id === $match->team_a_id) {
                        $teamAScore = $scoreText;
                    }

                    if ($sb->team_id === $match->team_b_id) {
                        $teamBScore = $scoreText;
                    }
                }

                /* ===============================
               STATUS + RESULT
            ================================*/
                $status = ucfirst($match->status);

                $resultText = match ($status) {
                    'Completed' => $match->result_summary ?? 'Match Completed',
                    'Live' => 'Match in progress',
                    default => Carbon::parse($match->match_date)->format('d M Y, h:i A')
                };

                return [
                    'id' => $match->id,

                    'title' => $match->title,
                    'tournament' => $match->tournament?->name,
                    'venue' => $match->venue,

                    'team_a' => $match->teamA?->name,
                    'team_b' => $match->teamB?->name,
                    'team_a_score' => $teamAScore,
                    'team_b_score' => $teamBScore,

                    'match_date' => Carbon::parse($match->match_date)->format('d M, Y'),
                    'match_type' => ucfirst($match->match_type),
                    'status' => $status,
                    'max_overs' => $match->max_overs,

                    'winning_team' => $match->winningTeam?->name,
                    'result_summary' => $resultText,

                    // Permissions
                    'canView' => Auth::user()->can($this->module . '-view'),
                    'canScore' => $this->canManageScoreboard(),
                    'canEdit' => Auth::user()->can($this->module . '-edit'),
                    'canDelete' => Auth::user()->can($this->module . '-delete'),

                    // URLs
                    'viewUrl' => $viewUrl,
                    'startUrl' => $startUrl,
                    'editUrl' => $editUrl,
                ];
            });

            return response()->json([
                'success' => true,
                'data' => $formattedData
            ]);
        } catch (Exception $e) {

            Log::error("Error loading cricket matches", [
                'line' => $e->getLine(),
                'file' => $e->getFile(),
                'message' => $e->getMessage()
            ]);

            return response()->json([
                'success' => false,
                'message' => 'Error loading matches.'
            ], 500);
        }
    }

    public function create(Request $request)
    {
        try {
            if (Auth::user() && !Auth::user()->can($this->module . '-create')) {
                throw new Exception('Unauthorized Access');
            }

            session([
                'title' => 'Create Cricket Match',
                'breadcrumbs' => [
                    'home' => [
                        'url' => route('admin.dashboard'),
                        'name' => 'Dashboard'
                    ],
                    $this->module => [
                        'url' => route('admin.cricket-matches.index'),
                        'name' => 'Daily Cricket Matches Management'
                    ],
                    'create' => [
                        'url' => route('admin.cricket-matches.create'),
                        'name' => 'Create Match'
                    ],
                ]
            ]);

            $today = Carbon::today();
            $teamsInFutureTournaments = TournamentGroupTeam::whereHas('group.tournament', function ($query) use ($today) {
                $query->where('start_date', '>', $today);
            })->pluck('team_id')->toArray();

            $teams = Team::whereNotIn('id', $teamsInFutureTournaments)->orderBy('name')->get();
            $tournaments = Tournament::where('start_date', '>=', $today)->orderBy('start_date')->get();

            return view('admin.pages.cricket-matches.create', compact('teams', 'tournaments'));
        } catch (Exception $e) {
            Log::error("Error loading cricket match create form", [
                'line' => $e->getLine(),
                'file' => $e->getFile(),
                'message' => $e->getMessage(),
            ]);

            return redirect()->back()->with([
                'success' => false,
                'message' => 'Failed to load match creation form.',
            ]);
        }
    }

    public function edit(Request $request)
    {
        try {
            if (Auth::user() && !Auth::user()->can($this->module . '-edit')) {
                throw new Exception('Unauthorized Access');
            }

            $matchId = $request->query('cricket-match');

            if (!$matchId) {
                throw new Exception('No match ID provided.');
            }

            $match = CricketMatch::with(['teamA', 'teamB', 'tournament'])->findOrFail($matchId);
            $today = Carbon::today();
            $teamsInFutureTournaments = TournamentGroupTeam::whereHas('group.tournament', function ($query) use ($today) {
                $query->where('start_date', '>', $today);
            })->pluck('team_id')->toArray();
            $tournaments = Tournament::where('start_date', '>', $today)->get();
            $teams = Team::whereNotIn('id', $teamsInFutureTournaments)->get();

            return view('admin.pages.cricket-matches.edit', compact('match', 'teams', 'tournaments'));
        } catch (ModelNotFoundException $e) {
            Log::error("Cricket match not found", [
                'message' => $e->getMessage(),
                'match_id' => $request->query('cricket-match'),
            ]);
            return redirect()->back()->with([
                'success' => false,
                'message' => 'Match not found.',
            ]);
        } catch (Exception $e) {
            Log::error("Error loading cricket match edit form", [
                'line' => $e->getLine(),
                'file' => $e->getFile(),
                'message' => $e->getMessage(),
            ]);
            return redirect()->back()->with([
                'success' => false,
                'message' => 'Failed to load match for editing.',
            ]);
        }
    }

    public function store(Request $request)
    {
        try {
            if (Auth::user() && !Auth::user()->can($this->module . '-create')) {
                throw new Exception('Unauthorized Access');
            }

            $validator = Validator::make($request->all(), [
                'title' => 'nullable|string|max:255',
                'venue' => 'nullable|string|max:255',
                'team_a_id' => 'required|exists:teams,id|different:team_b_id',
                'team_b_id' => 'required|exists:teams,id|different:team_a_id',
                'match_date' => 'nullable|date',
                'max_overs' => 'nullable|integer|min:1',
                'bowler_max_overs' => 'nullable|integer|min:1',
                'match_type' => 'required|in:tournament,regular',
                'status' => 'required|in:live,upcoming,completed',
            ]);

            if ($validator->fails()) {
                return redirect()->back()->withErrors($validator)->withInput();
            }

            DB::beginTransaction();

            $teamA = Team::findOrFail($request->team_a_id);
            $teamB = Team::findOrFail($request->team_b_id);

            $matchTitle = $request->title ?: $teamA->name . ' vs ' . $teamB->name;

            $match = new CricketMatch();
            $match->title = $matchTitle;
            $match->venue = $request->venue;
            $match->team_a_id = $request->team_a_id;
            $match->team_b_id = $request->team_b_id;
            $match->match_date = $request->match_date;
            $match->max_overs = $request->max_overs;
            $match->match_type = $request->match_type;
            $match->status = $request->status;
            $match->bowler_max_overs = $request->bowler_max_overs;
            $match->save();

            DB::commit();

            return redirect()->route('admin.cricket-matches.index')->with([
                'success' => true,
                'message' => 'Match created successfully!',
            ]);
        } catch (Exception $e) {
            DB::rollBack();
            Log::error("Error storing cricket match", [
                'line' => $e->getLine(),
                'file' => $e->getFile(),
                'message' => $e->getMessage(),
            ]);

            return redirect()->back()->withInput()->with([
                'success' => false,
                'message' => 'An error occurred while creating the match.',
                'error' => $e->getMessage(),
            ]);
        }
    }

    public function update(Request $request, $id)
    {
        try {
            if (Auth::user() && !Auth::user()->can($this->module . '-edit')) {
                throw new Exception('Unauthorized Access');
            }

            $matchId = $request->query('cricket-match');

            if (!$matchId) {
                throw new Exception('No match ID provided.');
            }

            $match = CricketMatch::with(['teamA', 'teamB', 'tournament'])->findOrFail($matchId);
            $today = Carbon::today();
            $teamsInFutureTournaments = TournamentGroupTeam::whereHas('group.tournament', function ($query) use ($today) {
                $query->where('start_date', '>', $today);
            })->pluck('team_id')->toArray();
            $tournaments = Tournament::where('start_date', '>', $today)->get();
            $teams = Team::whereNotIn('id', $teamsInFutureTournaments)->get();

            return view('admin.pages.cricket-matches.edit', compact('match', 'teams', 'tournaments'));
        } catch (ModelNotFoundException $e) {
            Log::error("Cricket match not found", [
                'message' => $e->getMessage(),
                'match_id' => $request->query('cricket-match'),
            ]);
            return redirect()->back()->with([
                'success' => false,
                'message' => 'Match not found.',
            ]);
        } catch (Exception $e) {
            Log::error("Error loading cricket match edit form", [
                'line' => $e->getLine(),
                'file' => $e->getFile(),
                'message' => $e->getMessage(),
            ]);
            return redirect()->back()->with([
                'success' => false,
                'message' => 'Failed to load match for editing.',
            ]);
        }
    }

    public function storeToss(Request $request)
    {
        try {
            if (!$this->canAny([$this->module . '-toss', $this->module . '-scoreboard', 'scoreboard-edit'])) {
                return $this->unauthorizedJson();
            }

            $validation = Validator::make($request->all(), [
                'match_id' => 'required|exists:cricket_matches,id',
                'toss_winner_team_id' => 'required|exists:teams,id',
                'toss_decision' => 'required|in:bat,bowl,BAT,BOWL',
            ]);

            if ($validation->fails()) {
                return response()->json([
                    'success' => false,
                    'message' => 'Validation failed',
                    'errors' => $validation->errors(),
                ], 422);
            }

            $matchId = $request->input('match_id');

            // Update or create toss data
            $tossData = CricketMatchToss::updateOrCreate(
                ['cricket_match_id' => $matchId],
                [
                    'toss_winner_team_id' => $request->input('toss_winner_team_id'),
                    'decision' => strtolower($request->input('toss_decision')),
                ]
            );

            $match = CricketMatch::findOrFail($matchId);
            $teamA = $match->team_a_id;
            $teamB = $match->team_b_id;

            $tossWinner = (int) $request->input('toss_winner_team_id');
            $tossDecision = strtolower($request->input('toss_decision'));

            if ($tossDecision === 'bat') {
                $battingFirstTeam = $tossWinner;
            } else {
                $battingFirstTeam = ($tossWinner === $teamA) ? $teamB : $teamA;
            }

            $bowlingFirstTeam = ($battingFirstTeam === $teamA) ? $teamB : $teamA;

            MatchScoreBoard::where('match_id', $matchId)->delete();

            MatchScoreBoard::create([
                'match_id' => $matchId,
                'team_id' => $battingFirstTeam,
                'innings' => 1,
                'runs' => 0,
                'wickets' => 0,
                'overs' => 0,
                'status' => 'running'
            ]);

            MatchScoreBoard::create([
                'match_id' => $matchId,
                'team_id' => $bowlingFirstTeam,
                'innings' => 2,
                'runs' => 0,
                'wickets' => 0,
                'overs' => 0,
                'status' => 'waiting'
            ]);

            $is_tournament = $match->tournament ? true : false;

            event(new CricketMatchTossEvent($match, $tossData, $battingFirstTeam, $bowlingFirstTeam, $is_tournament));

            return response()->json([
                'success' => true,
                'message' => 'Toss data stored successfully.',
                'data' => $tossData,
                'matchScoreBoard' => MatchScoreBoard::where('match_id', $matchId)->get(),
                'batting_team_id' => $battingFirstTeam,
                'bowling_team_id' => $bowlingFirstTeam,
                'batting_team_name' => ($battingFirstTeam === $teamA) ? $match->teamA->name : $match->teamB->name,
                'bowling_team_name' => ($bowlingFirstTeam === $teamA) ? $match->teamA->name : $match->teamB->name,
                'toss_winner_team_name' => ($tossWinner === $teamA) ? $match->teamA->name : $match->teamB->name,
            ]);
        } catch (\Exception $e) {
            Log::error("Error storing toss data", [
                'line' => $e->getLine(),
                'file' => $e->getFile(),
                'message' => $e->getMessage(),
            ]);

            return response()->json([
                'success' => false,
                'message' => 'Failed to store toss data.',
            ], 500);
        }
    }

    public function viewScoreBoard($id)
    {
        try {
            if (!$this->canViewScoreboard()) {
                throw new Exception('Unauthorized Access');
            }

            $match = CricketMatch::with(['teamA', 'teamB', 'tournament'])->findOrFail($id);

            return view('admin.pages.cricket-matches.scoreboard', compact('match'))->with([
                'success' => true,
                'message' => 'Match Scoreboard Showing.',
            ]);
        } catch (ModelNotFoundException $e) {
            Log::error("Cricket match not found", [
                'message' => $e->getMessage(),
                'match_id' => $id,
            ]);
            return redirect()->back()->with([
                'success' => false,
                'message' => 'Match not found.',
            ]);
        } catch (Exception $e) {
            Log::error("Error starting cricket match", [
                'line' => $e->getLine(),
                'file' => $e->getFile(),
                'message' => $e->getMessage(),
            ]);
            return redirect()->back()->with([
                'success' => false,
                'message' => 'Failed to start match.',
            ]);
        }
    }

    public function startCricketMatch($id)
    {
        try {
            if (Auth::user() && !Auth::user()->can($this->module . '-start')) {
                throw new Exception('Unauthorized Access');
            }

            $match = CricketMatch::with(['teamA', 'teamB', 'tournament'])->findOrFail($id);

            // Update status dynamically from request (JS sends it)
            $match->status = request()->get('status', 'live');
            $match->save();

            if ($match->tournament) {
                $tournamentId = $match->tournament->id;

                // Team A
                TournamentTeamStat::firstOrCreate(
                    ['tournament_id' => $tournamentId, 'team_id' => $match->teamA->id],
                    ['matches_played' => 0, 'wins' => 0, 'losses' => 0, 'draws' => 0, 'points' => 0, 'nrr' => 0]
                );

                // Team B
                TournamentTeamStat::firstOrCreate(
                    ['tournament_id' => $tournamentId, 'team_id' => $match->teamB->id],
                    ['matches_played' => 0, 'wins' => 0, 'losses' => 0, 'draws' => 0, 'points' => 0, 'nrr' => 0]
                );
            }

            return response()->json([
                'success' => true,
                'message' => 'Match status updated successfully.'
            ]);
        } catch (ModelNotFoundException $e) {
            Log::error("Cricket match not found", [
                'message' => $e->getMessage(),
                'match_id' => $id,
            ]);
            return response()->json(['success' => false, 'message' => 'Match not found.']);
        } catch (Exception $e) {
            Log::error("Error starting cricket match", [
                'line' => $e->getLine(),
                'file' => $e->getFile(),
                'message' => $e->getMessage(),
            ]);
            return response()->json(['success' => false, 'message' => 'Failed to update match status.']);
        }
    }

    public function show($id)
    {
        if (!$this->canAny([$this->module . '-view'])) {
            abort(403, 'Unauthorized Access');
        }

        $match = CricketMatch::with(['teamA', 'teamB', 'winningTeam'])->findOrFail($id);

        // Fetch scoreboards
        $scoreboards = MatchScoreBoard::where('match_id', $id)
            ->orderBy('innings')
            ->with('team')
            ->get();

        $inningsData = [];

        foreach ($scoreboards as $sb) {
            $battingTeamId = $sb->team_id;
            $bowlingTeamId = ($battingTeamId == $match->team_a_id) ? $match->team_b_id : $match->team_a_id;

            // Get batting stats
            $batting = MatchPlayer::with('player.user')
                ->where('match_id', $id)
                ->where('team_id', $battingTeamId)
                ->get()
                ->filter(function ($player) {
                    return ($player->balls_faced ?? 0) > 0;
                })
                ->values();

            // Get bowling stats - check if data exists, if not, show message
            $bowling = MatchPlayer::with('player.user')
                ->where('match_id', $id)
                ->where('team_id', $bowlingTeamId)
                ->get()
                ->filter(function ($player) {
                    Log::info("Bowling Player Overs Bowled: " . $player->overs_bowled);
                    return ($player->overs_bowled ?? 0) > 0;
                })
                ->values();

            // Log if no bowling data found
            if ($bowling->count() === 0) {
                Log::warning("No bowling data found for innings {$sb->innings}, team {$bowlingTeamId}");
            }

            // Get partnerships
            $partnerships = Partnership::where('match_id', $id)
                ->where('team_id', $battingTeamId)
                ->with(['batter1.user', 'batter2.user'])
                ->get();

            // Get fall of wickets
            $fallOfWickets = FallOfWicket::where('match_id', $id)
                ->where('team_id', $battingTeamId)
                ->with('batter.user')
                ->orderBy('wicket_number')
                ->get();

            $inningsData[] = [
                'scoreboard' => $sb,
                'batting' => $batting,
                'bowling' => $bowling,
                'partnerships' => $partnerships,
                'fall_of_wickets' => $fallOfWickets,
                'has_bowling_data' => $bowling->count() > 0, // Add flag for view
            ];
        }

        $match_result = $match->status === 'completed' ? [
            'winning_team_id' => $match->winning_team_id,
            'winning_team' => $match->winningTeam?->name ?? 'Unknown',
            'summary' => $match->result_summary ?? '',
        ] : null;

        return view('admin.pages.cricket-matches.show', compact('match', 'inningsData', 'match_result'));
    }

    public function selectBatsman(Request $request)
    {
        try {
            if (!$this->canManageScoreboard()) {
                return $this->unauthorizedJson();
            }

            $validator = Validator::make($request->all(), [
                'match_id' => 'required|exists:cricket_matches,id',
                'team_id' => 'required|exists:teams,id',
                'player_id' => 'required|exists:players,id',
                'role' => 'required|in:on-strike,batting',
            ]);

            if ($validator->fails()) {
                return response()->json([
                    'success' => false,
                    'message' => 'Failed to select batsman.',
                    'errors' => $validator->errors()
                ]);
            }

            $matchId = $request->match_id;
            $teamId = $request->team_id;
            $playerId = $request->player_id;
            $role = $request->role;
            $scoreBoard = MatchScoreBoard::where('match_id', $matchId)->where('team_id', $teamId)->where('status', 'running')->first();
            $existingBatsmen = MatchPlayer::where('match_id', $matchId)->where('team_id', $teamId)->count();
            $status = $existingBatsmen === 0 ? 'on-strike' : $role;
            $currentInnings = $request->input('currentInnings');

            if (!$currentInnings) {
                $scoreBoard = MatchScoreBoard::where('match_id', $matchId)
                    ->where('team_id', $teamId)
                    ->where('status', 'running')
                    ->first();

                $currentInnings = $scoreBoard ? $scoreBoard->innings : 1;
            }

            $matchPlayer = MatchPlayer::updateOrCreate(
                [
                    'match_id' => $matchId,
                    'team_id' => $teamId,
                    'player_id' => $playerId,
                ],
                [
                    'runs_scored' => 0,
                    'balls_faced' => 0,
                    'wickets_taken' => 0,
                    'overs_bowled' => 0,
                    'status' => $status,
                ]
            );

            $playerStat = PlayerStat::firstOrNew(['player_id' => $playerId]);
            if (!$playerStat->exists) {
                $playerStat->total_runs = 0;
                $playerStat->balls_faced = 0;
                $playerStat->fours = 0;
                $playerStat->sixes = 0;
                $playerStat->strike_rate = 0.0;
                $playerStat->average = 0.0;
                $playerStat->innings_bowled = 0;
                $playerStat->overs_bowled = 0;
                $playerStat->runs_conceded = 0;
                $playerStat->wickets = 0;
                $playerStat->bowling_average = 0.0;
                $playerStat->economy_rate = 0.0;
                $playerStat->catches = 0;
                $playerStat->runouts = 0;
                $playerStat->stumpings = 0;
                $playerStat->matches_played = 0;
                $playerStat->innings_batted = 0;
            }

            if ($playerStat->last_match_id != $matchId) {
                $playerStat->matches_played += 1;
                $playerStat->last_match_id = $matchId;
            }

            if ($playerStat->last_batting_match_id != $matchId) {
                $playerStat->innings_batted += 1;
                $playerStat->last_batting_match_id = $matchId;
            }

            $playerStat->save();

            $activePartnership = Partnership::where('match_id', $matchId)
                ->where('team_id', $teamId)
                ->whereNull('wicket_id')
                ->where('innings', $currentInnings)
                ->latest()
                ->first();

            $player2Id = null;

            if (!$activePartnership) {
                $lastPartnership = Partnership::where('match_id', $matchId)
                    ->where('team_id', $teamId)
                    ->whereNotNull('wicket_id')
                    ->where('innings', $currentInnings)
                    ->latest()
                    ->first();

                if ($lastPartnership) {
                    $fallOfWicket = FallOfWicket::find($lastPartnership->wicket_id);

                    if ($fallOfWicket) {
                        if ($lastPartnership->batter_1_id && $lastPartnership->batter_1_id != $fallOfWicket->batter_id) {
                            $player2Id = $lastPartnership->batter_1_id;
                        } elseif ($lastPartnership->batter_2_id && $lastPartnership->batter_2_id != $fallOfWicket->batter_id) {
                            $player2Id = $lastPartnership->batter_2_id;
                        }
                    }
                }

                if ($player2Id) {
                    Partnership::create([
                        'match_id' => $matchId,
                        'team_id' => $teamId,
                        'batter_1_id' => $player2Id,
                        'batter_2_id' => $playerId,
                        'runs' => 0,
                        'balls' => 0,
                        'start_over' => 0.0,
                        'innings' => $currentInnings,
                    ]);
                }
            } else {
                if (!$activePartnership->batter_2_id) {
                    $activePartnership->update(['batter_2_id' => $playerId]);
                }
            }

            $cricketMatch = CricketMatch::with('tournament')->find($matchId);
            if ($cricketMatch && $cricketMatch->tournament) {
                $tournamentId = $cricketMatch->tournament->id;

                $playerTournamentStat = TournamentPlayerStat::firstOrNew([
                    'tournament_id' => $tournamentId,
                    'player_id' => $playerId,
                ]);

                if (!$playerTournamentStat->exists) {
                    $playerTournamentStat->matches_played = 0;
                    $playerTournamentStat->innings_batted = 0;
                    $playerTournamentStat->total_runs = 0;
                    $playerTournamentStat->balls_faced = 0;
                    $playerTournamentStat->fifties = 0;
                    $playerTournamentStat->hundreds = 0;
                    $playerTournamentStat->sixes = 0;
                    $playerTournamentStat->fours = 0;
                    $playerTournamentStat->strike_rate = 0.0;
                    $playerTournamentStat->average = 0.0;
                    $playerTournamentStat->innings_bowled = 0;
                    $playerTournamentStat->overs_bowled = 0;
                    $playerTournamentStat->runs_conceded = 0;
                    $playerTournamentStat->wickets = 0;
                    $playerTournamentStat->bowling_average = 0.0;
                    $playerTournamentStat->economy_rate = 0.0;
                    $playerTournamentStat->catches = 0;
                    $playerTournamentStat->runouts = 0;
                    $playerTournamentStat->stumpings = 0;
                }

                if ($playerTournamentStat->last_match_id != $matchId) {
                    $playerTournamentStat->matches_played += 1;
                    $playerTournamentStat->last_match_id = $matchId;
                }

                if ($playerTournamentStat->last_batting_match_id != $matchId) {
                    $playerTournamentStat->innings_batted += 1;
                    $playerTournamentStat->last_batting_match_id = $matchId;
                }

                $playerTournamentStat->save();
            }

            $match = CricketMatch::findOrFail($matchId);
            event(new CricketMatchUpdateEvent($match, $this->getBroadcastData($match)));

            return response()->json([
                'success' => true,
                'message' => 'Batsman selected successfully.',
                'data' => [
                    'match_player' => $matchPlayer,
                ]
            ]);
        } catch (\Exception $e) {
            Log::error("Error selecting batsman", [
                'line' => $e->getLine(),
                'file' => $e->getFile(),
                'message' => $e->getMessage(),
            ]);
            return response()->json([
                'success' => false,
                'message' => 'Failed to select batsman.',
            ], 500);
        }
    }

    public function selectBowler(Request $request)
    {
        try {
            if (!$this->canManageScoreboard()) {
                return $this->unauthorizedJson();
            }

            $validator = Validator::make($request->all(), [
                'match_id' => 'required|exists:cricket_matches,id',
                'team_id' => 'required|exists:teams,id',
                'player_id' => 'required|exists:players,id',
            ]);

            if ($validator->fails()) {
                Log::error($validator->errors());
                return response()->json([
                    'success' => false,
                    'message' => 'Failed to select bowler.',
                    'errors' => $validator->errors()
                ]);
            }

            $matchId = $request->match_id;
            $battingTeamId = $request->team_id;
            $playerId = $request->player_id;

            $scoreboard = MatchScoreBoard::where('match_id', $matchId)->get();

            if ($scoreboard->isEmpty()) {
                return response()->json([
                    'success' => false,
                    'message' => 'Scoreboard not found for this match.',
                ], 404);
            }

            $bowlingTeamId = $scoreboard
                ->where('team_id', '!=', $battingTeamId)
                ->where('status', 'ended')
                ->first()?->team_id;

            if (!$bowlingTeamId) {
                return response()->json([
                    'success' => false,
                    'message' => 'Bowling team not determined.',
                ], 400);
            }

            $existingPlayers = MatchPlayer::where('match_id', $matchId)
                ->where('team_id', $bowlingTeamId)
                ->get();

            foreach ($existingPlayers as $player) {
                $status = $player->player_id == $playerId ? 'bowling' : 'fielding';

                $player->update([
                    'status' => $status,
                ]);
            }

            $match = CricketMatch::findOrFail($matchId);
            event(new CricketMatchUpdateEvent($match, $this->getBroadcastData($match)));

            return response()->json([
                'success' => true,
                'message' => 'Bowler selected successfully.',
            ]);
        } catch (\Exception $e) {
            Log::error("Error selecting bowler", [
                'line' => $e->getLine(),
                'file' => $e->getFile(),
                'message' => $e->getMessage(),
            ]);

            return response()->json([
                'success' => false,
                'message' => 'Failed to select bowler.',
            ], 500);
        }
    }

    protected function mapPlayer($player)
    {
        return [
            'id' => $player->id,
            'name' => $player->user->full_name ?? 'Unknown Player',
            'overs_bowled' => $player->overs_bowled ?? 0,
            'image_url' => $player->user->image ?? asset('images/default-player.png'),
            'bowling_style' => ucfirst($player->bowling_style) ?? 'N/A',
        ];
    }

    public function getPlayersList(Request $request)
    {
        try {
            if (!$this->canViewScoreboard()) {
                return $this->unauthorizedJson();
            }

            $matchId = $request->match_id;
            $teamId = $request->team_id;
            $type = $request->type;
            $players = collect();

            if (!in_array($type, ['batting', 'bowling'])) {
                return response()->json([
                    'success' => false,
                    'message' => 'Invalid type provided'
                ], 422);
            }

            /* ===============================
            BATSMEN → YET TO BAT
            ================================*/
            if ($type === 'batting') {

                $battedPlayerIds = MatchPlayer::where('match_id', $matchId)
                    ->where('team_id', $teamId)
                    ->where(function ($q) {
                        $q->whereNotNull('runs_scored')
                            ->orWhereNotNull('balls_faced');
                    })
                    ->whereNotIn('status', ['ready', 'retired-hurt'])
                    ->pluck('player_id');

                $players = MatchPlayer::with('player.user')
                    ->where('match_id', $matchId)
                    ->where('team_id', $teamId)
                    ->whereNotIn('player_id', $battedPlayerIds)
                    ->get()
                    ->map(fn($mp) => $this->mapPlayer($mp));
            }

            /* ===============================
            BOWLERS → QUOTA LEFT
            ================================*/
            if ($type === 'bowling') {
                $match = CricketMatch::find($matchId);
                $maxOversPerBowler = $match->bowler_max_overs;

                $players = Player::whereHas('teams', function ($q) use ($teamId) {
                    $q->where('teams.id', $teamId);
                })->get()->map(function ($player) use ($matchId, $teamId, $maxOversPerBowler) {
                    $matchPlayer = MatchPlayer::where('match_id', $matchId)
                        ->where('team_id', $teamId)
                        ->where('player_id', $player->id)
                        ->first();

                    $oversBowled = $matchPlayer ? $matchPlayer->overs_bowled : 0;

                    // Only include players who haven't exceeded max overs
                    if ($oversBowled < $maxOversPerBowler) {
                        // Attach overs_bowled so mapPlayer can use it
                        $player->overs_bowled = $oversBowled;
                        return $player;
                    }
                })->filter()->values(); // Remove null values

                $players = $players->map(fn($player) => $this->mapPlayer($player));
            }

            return response()->json([
                'success' => true,
                'data' => $players,
            ]);
        } catch (\Exception $e) {

            Log::error('Error fetching players list', [
                'message' => $e->getMessage(),
                'line' => $e->getLine(),
            ]);

            return response()->json([
                'success' => false,
                'message' => 'Failed to fetch players list',
            ], 500);
        }
    }

    public function getFullMatchState(Request $request)
    {
        try {
            if (!$this->canViewScoreboard()) {
                return $this->unauthorizedJson();
            }

            $matchId = $request->match_id ?? $request->route('match_id');
            $match = CricketMatch::findOrFail($matchId);

            // --- Shared: load scoreboards ---
            $scoreboards = MatchScoreBoard::where('match_id', $matchId)
                ->orderBy('innings')
                ->get();

            if ($scoreboards->isEmpty()) {
                return response()->json([
                    'success' => true,
                    'match_id' => $matchId,
                    'innings' => [],
                    'match_state' => [
                        'striker' => null,
                        'nonStriker' => null,
                        'team' => null,
                        'currentBowler' => null,
                        'battingTeamId' => null,
                        'bowlingTeamId' => null,
                        'currentInnings' => null,
                    ],
                    'match_result' => $match->status === 'completed' ? [
                        'winning_team_id' => $match->winning_team_id,
                        'winning_team' => $match->winningTeam?->name ?? 'Unknown',
                        'summary' => $match->result_summary ?? '',
                    ] : null,
                    'message' => 'No toss yet, scoreboard not started.',
                ]);
            }

            // --- Part 1: Build innings stats (same as loadCurrentStats) ---
            $allInnings = [];
            foreach ($scoreboards as $scoreboard) {
                $inningsNo = $scoreboard->innings;
                $batting_team_id = $scoreboard->team_id;
                $bowling_team_id = ($batting_team_id == $match->team_a_id) ? $match->team_b_id : $match->team_a_id;

                // Batting
                $batting = MatchPlayer::with('player.user')
                    ->where('match_id', $matchId)
                    ->where('team_id', $batting_team_id)
                    ->get()
                    ->map(function ($mp) {
                        $user = $mp->player?->user;
                        return [
                            'id' => $mp->player_id,
                            'name' => $user?->full_name ?? 'Unknown',
                            'runs' => $mp->runs_scored ?? 0,
                            'balls' => $mp->balls_faced ?? 0,
                            'fours' => $mp->fours ?? 0,
                            'sixes' => $mp->sixes ?? 0,
                            'status' => $mp->status ?? '---',
                            'strike_rate' => $mp->balls_faced
                                ? round(($mp->runs_scored / $mp->balls_faced) * 100, 2)
                                : 0,
                        ];
                    });

                // Bowling
                $bowling = MatchPlayer::with('player.user')
                    ->where('match_id', $matchId)
                    ->where('team_id', $bowling_team_id)
                    // ->where('overs_bowled', '>', 0)
                    ->get()
                    ->map(function ($mp) {
                        $user = $mp->player?->user;
                        $oversBowled = $mp->overs_bowled ?? 0;
                        $overPart = floor($oversBowled);
                        $ballPart = round(($oversBowled - $overPart) * 10);
                        $decimalOvers = $overPart + ($ballPart / 6);

                        return [
                            'id' => $mp->player_id,
                            'name' => $user?->full_name ?? 'Unknown',
                            'overs' => $mp->overs_bowled,
                            'runs_conceded' => $mp->runs_conceded,
                            'wickets' => $mp->wickets_taken,
                            'economy_rate' => $decimalOvers > 0
                                ? round($mp->runs_conceded / $decimalOvers, 2)
                                : 0,
                        ];
                    });

                // Partnerships
                $partnerships = Partnership::with(['batter1.user', 'batter2.user'])
                    ->where('match_id', $matchId)
                    ->where('team_id', $batting_team_id)
                    ->where('innings', $inningsNo)
                    ->get()
                    ->map(function ($p) {
                        $totalRuns = max(1, $p->player1_runs + $p->player2_runs);
                        $batter1Percent = round(($p->player1_runs / $totalRuns) * 100);
                        $batter2Percent = 100 - $batter1Percent;

                        return [
                            'batter1' => [
                                'id' => $p->batter_1_id,
                                'name' => $p->batter1?->user?->full_name ?? 'Unknown',
                                'role' => strtoupper(str_replace('-', ' ', $p->batter1?->batting_style)) ?? 'Unknown',
                                'runs' => $p->player1_runs,
                                'balls' => $p->batter_1_balls ?? 0,
                                'percent' => $batter1Percent,
                                'img' => $p->batter1?->user?->image ?? 'https://coffective.com/wp-content/uploads/2018/06/default-featured-image.png.jpg',
                            ],
                            'batter2' => $p->batter2 ? [
                                'id' => $p->batter_2_id,
                                'name' => $p->batter2?->user?->full_name ?? 'Unknown',
                                'role' => strtoupper(str_replace('-', ' ', $p->batter2?->batting_style)) ?? 'Unknown',
                                'runs' => $p->player2_runs,
                                'balls' => $p->batter_2_balls ?? 0,
                                'percent' => $batter2Percent,
                                'img' => $p->batter1?->user?->image ?? 'https://coffective.com/wp-content/uploads/2018/06/default-featured-image.png.jpg',
                            ] : null,
                            'runs' => $p->runs,
                            'balls' => $p->balls,
                        ];
                    });

                // Fall of wickets
                $fallOfWickets = FallOfWicket::with('batter.user')
                    ->where('match_id', $matchId)
                    ->where('team_id', $batting_team_id)
                    ->where('innings', $inningsNo)
                    ->orderBy('wicket_number')
                    ->get()
                    ->map(function ($w) {
                        return [
                            'player_name' => $w->batter?->user?->full_name ?? 'Unknown',
                            'runs' => $w->runs,
                            'over' => $w->overs,
                            'wicket_number' => $w->wicket_number,
                            'dismissal_type' => $w->dismissal_type,
                        ];
                    });

                $totalOvers = $match->max_overs;
                $oversParts = explode('.', $scoreboard->overs ?? '0.0');
                $completedOvers = intval($oversParts[0]);
                $balls = isset($oversParts[1]) ? intval($oversParts[1]) : 0;
                $oversBowled = $completedOvers + ($balls / 6);

                $currentRate = $oversBowled > 0
                    ? round($scoreboard->runs / $oversBowled, 2)
                    : 0;
                $projected = round($currentRate * $totalOvers);

                $targetScore = 0;
                $requiredRunRate = 0;

                if ($inningsNo == 2) {
                    $firstInnings = $scoreboards->where('innings', 1)->first();
                    if ($firstInnings && $firstInnings->status == 'ended') {
                        $targetScore = $firstInnings->runs + 1;
                        $oversLeft = $totalOvers - $oversBowled;
                        $requiredRunRate = $oversLeft > 0 ? round(($targetScore - $scoreboard->runs) / $oversLeft, 2) : 0;
                    }
                }

                $allInnings[] = [
                    'innings' => $inningsNo,
                    'batting_team_id' => $batting_team_id,
                    'bowling_team_id' => $bowling_team_id,
                    'scoreboard' => [
                        'runs' => $scoreboard->runs ?? 0,
                        'wickets' => $scoreboard->wickets ?? 0,
                        'overs' => $scoreboard->overs ?? '0.0',
                        'status' => $scoreboard->status,
                        'totalOvers' => $totalOvers,
                        'currentCRR' => $currentRate,
                        'projected' => $projected,
                        'target' => $targetScore,
                        'requiredRR' => $requiredRunRate,
                    ],
                    'batting' => $batting,
                    'bowling' => $bowling,
                    'partnerships' => $partnerships,
                    'fall_of_wickets' => $fallOfWickets,
                ];
            }

            // --- Part 2: Match state (from getMatchInfo) ---
            $runningScoreboard = $scoreboards->where('status', 'running')->first() ?? $scoreboards->last();
            $batting_team_id = $runningScoreboard->team_id;
            $bowling_team_id = ($batting_team_id == $match->team_a_id) ? $match->team_b_id : $match->team_a_id;

            $battingTeam = Team::find($batting_team_id);
            $striker = MatchPlayer::where('match_id', $matchId)
                ->where('team_id', $batting_team_id)
                ->where('status', 'on-strike')
                ->first();
            $nonStriker = MatchPlayer::where('match_id', $matchId)
                ->where('team_id', $batting_team_id)
                ->where('status', 'batting')
                ->first();
            $currentBowler = MatchPlayer::where('match_id', $matchId)
                ->where('team_id', $bowling_team_id)
                ->where('status', 'bowling')
                ->orderByDesc('overs_bowled')
                ->first();
            $bowlingTeamPlayers = MatchPlayer::where('match_id', $matchId)
                ->where('team_id', $bowling_team_id)
                ->whereIn('status', ['bowling', 'fielding'])
                ->with('player.user')
                ->get()
                ->map(function ($mp) use ($match) {
                    $maxOvers = (int) ($match->bowler_max_overs ?? 0);
                    return [
                        'id' => $mp->player_id,
                        'name' => $mp->player?->user?->full_name ?? 'Unknown',
                        'role' => $mp->player?->player_role,
                        'status' => $mp->status,
                        'overs' => $mp->overs_bowled ?? 0,
                        'runs_conceded' => $mp->runs_conceded ?? 0,
                        'wickets' => $mp->wickets_taken ?? 0,
                        'remaining_overs' => $maxOvers > 0 ? max(0, $maxOvers - (float) ($mp->overs_bowled ?? 0)) : null,
                    ];
                })
                ->values();

            $currentOverData = json_decode($this->getCurrentOver($matchId)->getContent(), true) ?: [];
            $currentOverBalls = $currentOverData['balls'] ?? [];
            $legalBallsInCurrentOver = (int) ($currentOverData['legalBalls'] ?? 0);
            $currentOversParts = explode('.', (string) ($runningScoreboard->overs ?? '0.0'));
            $currentTotalLegalBalls = ((int) ($currentOversParts[0] ?? 0) * 6) + (int) ($currentOversParts[1] ?? 0);
            $currentInningsMaxBalls = (int) ($match->max_overs ?? 0) * 6;

            if ($legalBallsInCurrentOver >= 6 && ($currentInningsMaxBalls === 0 || $currentTotalLegalBalls < $currentInningsMaxBalls)) {
                $currentOverBalls = [];
            }

            $matchState = [
                'striker' => $striker ? [
                    'id' => $striker->player_id,
                    'name' => $striker->player->user->full_name ?? 'Unknown',
                    'runs' => $striker->runs_scored ?? 0,
                    'balls' => $striker->balls_faced ?? 0,
                ] : null,
                'nonStriker' => $nonStriker ? [
                    'id' => $nonStriker->player_id,
                    'name' => $nonStriker->player->user->full_name ?? 'Unknown',
                    'runs' => $nonStriker->runs_scored ?? 0,
                    'balls' => $nonStriker->balls_faced ?? 0,
                ] : null,
                'team' => $battingTeam ? [
                    'id' => $battingTeam->id,
                    'name' => $battingTeam->name,
                    'total_players' => $battingTeam->players()->count(),
                ] : null,
                'bowlingTeamPlayers' => $bowlingTeamPlayers,
                'currentBowler' => $currentBowler ? [
                    'id' => $currentBowler->player_id,
                    'name' => $currentBowler->player->user->full_name ?? 'Unknown',
                    'overs' => $currentBowler->overs_bowled ?? 0,
                    'runs_conceded' => $currentBowler->runs_conceded ?? 0,
                    'wickets' => $currentBowler->wickets_taken ?? 0,
                ] : null,
                'battingTeamId' => $batting_team_id,
                'bowlingTeamId' => $bowling_team_id,
                'currentInnings' => $runningScoreboard->innings,
                'inningsStatus' => $runningScoreboard->status,
                'currentOver' => [
                    'over_number' => $currentOverData['current_over'] ?? null,
                    'balls' => $currentOverBalls,
                    'legalBalls' => $legalBallsInCurrentOver,
                ],
            ];

            return response()->json([
                'success' => true,
                'match_id' => $matchId,
                'match_status' => $match->status,
                'max_overs' => $match->max_overs,
                'bowler_max_overs' => $match->bowler_max_overs,
                'innings' => $allInnings,
                'match_state' => $matchState,
                'match_result' => $match->status === 'completed' ? [
                    'winning_team_id' => $match->winning_team_id,
                    'winning_team' => $match->winningTeam?->name ?? 'Unknown',
                    'summary' => $match->result_summary ?? '',
                    'scoreboards' => [
                        'teamA' => $scoreboards->where('team_id', $match->team_a_id)->first()->load('team'),
                        'teamB' => $scoreboards->where('team_id', $match->team_b_id)->first()->load('team'),
                    ],
                ] : null,
            ]);
        } catch (\Exception $e) {
            Log::error('Error loading match state X', [
                'message' => $e->getMessage(),
                'file' => $e->getFile(),
                'line' => $e->getLine()
            ]);

            return response()->json([
                'success' => false,
                'message' => 'Failed to load full match state.'
            ]);
        }
    }

    public function getTeamBPlayers($matchId)
    {
        if (!$this->canViewScoreboard()) {
            return $this->unauthorizedJson();
        }

        $match = CricketMatch::with(['teamB.players.user'])->findOrFail($matchId);

        $players = $match->teamB->players->map(function ($player) {
            return [
                'id' => $player->id,
                'name' => $player->user->full_name,
                'style' => $player->bowling_style ?? 'N/A'
            ];
        });

        return response()->json($players);
    }

    public function chooseBowler(Request $request)
    {
        try {
            if (!$this->canManageScoreboard()) {
                return $this->unauthorizedJson();
            }

            $validator = Validator::make($request->all(), [
                'match_id' => 'required|exists:cricket_matches,id',
                'bowler_id' => 'required|exists:players,id',
                'team_id' => 'required|exists:teams,id',
                'style' => 'nullable|string'
            ]);

            if ($validator->fails()) {
                return response()->json([
                    'success' => false,
                    'message' => 'Invalid data',
                    'errors' => $validator->errors()
                ], 422);
            }

            $matchId = $request->match_id;
            $bowlerId = $request->bowler_id;
            $teamId = $request->team_id;

            $match = CricketMatch::find($matchId);

            DB::transaction(function () use ($matchId, $bowlerId, $teamId, $match) {
                MatchPlayer::where('match_id', $matchId)
                    ->where('team_id', $teamId)
                    ->where('player_id', '<>', $bowlerId)
                    ->update(['status' => 'fielding']);

                MatchPlayer::updateOrCreate(
                    [
                        'match_id' => $matchId,
                        'player_id' => $bowlerId
                    ],
                    array_merge(
                        ['team_id' => $teamId, 'status' => 'bowling'], // always set
                        MatchPlayer::where('match_id', $matchId)->where('player_id', $bowlerId)->exists()
                        ? []
                        : [
                            'overs_bowled' => 0,
                            'runs_conceded' => 0,
                            'wickets_taken' => 0,
                            'maidens' => 0,
                        ]
                    )
                );

                $playerStat = PlayerStat::firstOrNew(['player_id' => $bowlerId]);

                if (!$playerStat->exists) {
                    $playerStat->matches_played = 0;
                    $playerStat->innings_batted = 0;
                    $playerStat->total_runs = 0;
                    $playerStat->balls_faced = 0;
                    $playerStat->fifties = 0;
                    $playerStat->hundreds = 0;
                    $playerStat->sixes = 0;
                    $playerStat->fours = 0;
                    $playerStat->strike_rate = 0;
                    $playerStat->average = 0;
                    $playerStat->innings_bowled = 0;
                    $playerStat->overs_bowled = 0;
                    $playerStat->runs_conceded = 0;
                    $playerStat->wickets = 0;
                    $playerStat->bowling_average = 0;
                    $playerStat->economy_rate = 0;
                    $playerStat->catches = 0;
                    $playerStat->runouts = 0;
                    $playerStat->stumpings = 0;
                }

                if ($playerStat->last_match_id != $matchId) {
                    $playerStat->matches_played += 1;
                    $playerStat->last_match_id = $matchId;
                }

                if ($playerStat->last_bowling_match_id != $matchId) {
                    $playerStat->innings_bowled += 1;
                    $playerStat->last_bowling_match_id = $matchId;
                }

                $playerStat->save();

                if ($match->tournament_id) {
                    $playerTournamentStat = TournamentPlayerStat::firstOrNew([
                        'tournament_id' => $match->tournament_id,
                        'player_id' => $bowlerId
                    ]);

                    if (!$playerTournamentStat->exists) {
                        $playerTournamentStat->matches_played = 0;
                        $playerTournamentStat->innings_batted = 0;
                        $playerTournamentStat->total_runs = 0;
                        $playerTournamentStat->balls_faced = 0;
                        $playerTournamentStat->fifties = 0;
                        $playerTournamentStat->hundreds = 0;
                        $playerTournamentStat->sixes = 0;
                        $playerTournamentStat->fours = 0;
                        $playerTournamentStat->strike_rate = 0;
                        $playerTournamentStat->average = 0;
                        $playerTournamentStat->innings_bowled = 0;
                        $playerTournamentStat->overs_bowled = 0;
                        $playerTournamentStat->runs_conceded = 0;
                        $playerTournamentStat->wickets = 0;
                        $playerTournamentStat->bowling_average = 0;
                        $playerTournamentStat->economy_rate = 0;
                        $playerTournamentStat->catches = 0;
                        $playerTournamentStat->runouts = 0;
                        $playerTournamentStat->stumpings = 0;
                    }

                    if ($playerTournamentStat->last_match_id != $matchId) {
                        $playerTournamentStat->matches_played += 1;
                        $playerTournamentStat->last_match_id = $matchId;
                    }

                    if ($playerTournamentStat->last_bowling_match_id != $matchId) {
                        $playerTournamentStat->innings_bowled += 1;
                        $playerTournamentStat->last_bowling_match_id = $matchId;
                    }

                    $playerTournamentStat->save();
                }
            });

            $bowling = MatchPlayer::with(['player.user'])
                ->where('match_id', $matchId)
                ->where('team_id', $teamId)
                ->where('player_id', $bowlerId)
                ->whereIn('status', ['bowling', 'fielding'])
                ->first();

            if ($bowling) {
                $bowling = [
                    'id' => $bowling->player_id,
                    'name' => optional(optional($bowling->player)->user)->full_name ?? 'Unknown',
                    'style' => optional($bowling->player)->bowling_style ?? 'Unknown',
                    'overs' => $bowling->overs_bowled ?? 0,
                    'runs_conceded' => $bowling->runs_conceded ?? 0,
                    'wickets' => $bowling->wickets_taken ?? 0,
                    'economy_rate' => $bowling->overs_bowled > 0
                        ? round($bowling->runs_conceded / $bowling->overs_bowled, 2)
                        : 0,
                    'maidens' => $bowling->maidens ?? 0
                ];
            } else {
                $bowling = null;
            }

            event(new CricketMatchUpdateEvent($match, $this->getBroadcastData($match)));

            return response()->json([
                'success' => true,
                'message' => 'Bowler selected successfully',
                'bowling' => $bowling
            ]);
        } catch (\Exception $e) {
            Log::error("Error Choosing Bowler: " . $e->getMessage(), [
                'line' => $e->getLine(),
                'file' => $e->getFile()
            ]);

            return response()->json([
                'success' => false,
                'message' => 'Failed to select bowler',
                'error' => $e->getMessage()
            ], 500);
        }
    }

    public function getCurrentOver($matchId)
    {
        try {
            if (!$this->canViewScoreboard()) {
                return $this->unauthorizedJson();
            }

            $scoreboard = MatchScoreBoard::where('match_id', $matchId)->where('status', 'running')->first();

            if (!$scoreboard) {
                return response()->json([
                    'success' => true,
                    'current_over' => null,
                    'balls' => [],
                    'legalBalls' => 6
                ]);
            }

            $lastDelivery = MatchDelivery::where('match_id', $matchId)
                ->where('innings', $scoreboard->innings)
                ->orderByDesc('id')
                ->first();

            $currentOverNumber = $lastDelivery ? $lastDelivery->over_number : 1;

            $deliveries = MatchDelivery::where('match_id', $matchId)
                ->where('over_number', $currentOverNumber)
                ->where('innings', $scoreboard->innings)
                ->orderBy('ball_in_over')
                ->get();

            $overBalls = $deliveries->map(function ($d) {
                $ballLabel = '';
                $class = 'ball';
                $titleLabel = '';

                switch ($d->delivery_type) {
                    case 'no-ball':
                        $class .= ' extra-ball';
                        $titleLabel = 'NB';
                        if ($d->runs_batsman > 0) {
                            $ballLabel = (string) $d->runs_batsman;
                        } elseif ($d->runs_extras > 0) {
                            $ballLabel = (string) $d->runs_extras;
                        } else {
                            $ballLabel = 'NB';
                        }

                        if ($d->is_wicket) {
                            $ballLabel = $ballLabel && $ballLabel !== '0'
                                ? $ballLabel . 'W'
                                : 'W';
                        }
                        break;

                    case 'wide':
                        $class .= ' run-ball';
                        $ballLabel = 'WD';
                        $titleLabel = 'WD';
                        if ($d->runs_extras)
                            $ballLabel .= $d->runs_extras;
                        if ($d->is_wicket)
                            $ballLabel .= 'W';
                        break;

                    case 'bye':
                        $class .= ' run-ball extra-ball';
                        $ballLabel = 'B';
                        $titleLabel = 'BY';
                        if ($d->runs_extras)
                            $ballLabel .= $d->runs_extras;
                        if ($d->is_wicket)
                            $ballLabel .= 'W';
                        break;

                    case 'leg-bye':
                        $class .= ' run-ball extra-ball';
                        $ballLabel = 'LB';
                        $titleLabel = 'LB';
                        if ($d->runs_extras)
                            $ballLabel .= $d->runs_extras;
                        if ($d->is_wicket)
                            $ballLabel .= 'W';
                        break;

                    default:
                        if ($d->runs_batsman > 0) {
                            $ballLabel = (string) $d->runs_batsman;
                            $class .= $d->runs_batsman == 4 ? ' four-ball'
                                : ($d->runs_batsman == 6 ? ' six-ball' : ' run-ball');
                        } elseif ($d->runs_batsman == 0 && $d->runs_extras == 0) {
                            $ballLabel = '0';
                            $class .= ' dot-ball';
                        } else {
                            $ballLabel = (string) $d->runs_batsman;
                        }

                        if ($d->is_wicket) {
                            $ballLabel = $ballLabel && $ballLabel !== '0'
                                ? $ballLabel . 'W'
                                : 'W';
                        }
                        break;
                }

                return [
                    'ball' => $ballLabel,
                    'class' => $class,
                    'type' => $d->delivery_type,
                    'title' => $titleLabel
                ];
            });

            return response()->json([
                'success' => true,
                'current_over' => $currentOverNumber,
                'balls' => $overBalls,
                'legalBalls' => $deliveries->whereIn('delivery_type', ['normal', 'bye', 'leg-bye'])->count()
            ]);
        } catch (\Exception $e) {
            Log::error('Error fetching current over', ['message' => $e->getMessage()]);
            return response()->json(['success' => false, 'message' => 'Failed to fetch current over.']);
        }
    }

    public function switchStrike(Request $request)
    {
        try {
            if (!$this->canManageScoreboard()) {
                return $this->unauthorizedJson();
            }

            $validator = Validator::make($request->all(), [
                'match_id' => 'required|exists:cricket_matches,id',
            ]);

            if ($validator->fails()) {
                return response()->json([
                    'success' => false,
                    'message' => 'Invalid request.',
                    'errors' => $validator->errors(),
                ], 422);
            }

            $matchId = $request->match_id;
            $strikeSwitched = $this->doSwitchStrike($matchId);

            // Get current striker and non-striker
            $striker = MatchPlayer::where('match_id', $matchId)
                ->where('status', 'on-strike')
                ->first();

            $nonStriker = MatchPlayer::where('match_id', $matchId)
                ->where('status', 'batting')
                ->first();

            if (!$striker || !$nonStriker) {
                return response()->json([
                    'success' => false,
                    'message' => 'Cannot switch strike. Both striker and non-striker must be selected.',
                ], 400);
            }

            if (!$strikeSwitched) {
                return response()->json([
                    'success' => false,
                    'message' => 'Strike switch failed.',
                ], 500);
            }

            return response()->json([
                'success' => true,
                'message' => 'Strike switched successfully.',
                'data' => [
                    'striker' => [
                        'id' => $striker->player_id,
                        'name' => $striker->player->user->full_name,
                        'img' => $striker->player->image ?? asset((request()->getHost() === 'app.madcricketers.com' ? 'public/' : '') . 'storage/assets/images/users/dummy-avatar.jpg'),
                        'runs' => $striker->runs_scored ?? 0,
                        'balls' => $striker->balls_faced ?? 0,
                    ],
                    'nonStriker' => [
                        'id' => $nonStriker->player_id,
                        'name' => $nonStriker->player->user->full_name,
                        'img' => $nonStriker->player->image ?? asset((request()->getHost() === 'app.madcricketers.com' ? 'public/' : '') . 'storage/assets/images/users/dummy-avatar.jpg'),
                        'runs' => $nonStriker->runs_scored ?? 0,
                        'balls' => $nonStriker->balls_faced ?? 0,
                    ]
                ]
            ]);
        } catch (\Exception $e) {
            Log::error("Error switching strike", [
                'line' => $e->getLine(),
                'file' => $e->getFile(),
                'message' => $e->getMessage(),
            ]);

            return response()->json([
                'success' => false,
                'message' => 'Failed to switch strike.',
            ], 500);
        }
    }

    private function doSwitchStrike($matchId)
    {
        // Get the current batting team
        $scoreboard = MatchScoreBoard::where('match_id', $matchId)
            ->where('status', 'running')
            ->first();

        if (!$scoreboard) {
            return false;
        }

        $battingTeamId = $scoreboard->team_id;

        // Get all active batters (not dismissed)
        $activeBatters = MatchPlayer::where('match_id', $matchId)
            ->where('team_id', $battingTeamId)
            ->whereIn('status', ['on-strike', 'batting'])
            ->get();

        if ($activeBatters->count() < 2) {
            return false;
        }

        // Find current striker and non-striker
        $striker = $activeBatters->firstWhere('status', 'on-strike');
        $nonStriker = $activeBatters->firstWhere('status', 'batting');

        // If we can't find both with expected statuses, try to find any two active batters
        if (!$striker || !$nonStriker) {
            $activeBatters = $activeBatters->take(2);
            if ($activeBatters->count() < 2) {
                return false;
            }
            $striker = $activeBatters->first();
            $nonStriker = $activeBatters->last();
        }

        Log::info('Before switch - Striker: ' . $striker->player_id . ' (' . $striker->status . '), Non-striker: ' . $nonStriker->player_id . ' (' . $nonStriker->status . ')');

        // Swap their statuses
        $striker->status = ($striker->status === 'on-strike') ? 'batting' : 'on-strike';
        $nonStriker->status = ($nonStriker->status === 'on-strike') ? 'batting' : 'on-strike';

        $striker->save();
        $nonStriker->save();

        Log::info('After switch - Striker: ' . $nonStriker->player_id . ', Non-striker: ' . $striker->player_id);

        return true;
    }

    protected function switchTeam($match, $scoreboard)
    {
        if ($scoreboard->innings >= 2) {
            return null;
        }

        $currentBattingTeamId = $scoreboard->team_id;
        $nextBattingTeamId = ($currentBattingTeamId == $match->team_a_id)
            ? $match->team_b_id
            : $match->team_a_id;

        $nextInnings = $scoreboard->innings + 1;

        if ($nextInnings == 2 && (float) $scoreboard->overs == $scoreboard->match->max_overs) {
            $scoreboard->update(['status' => 'ended']);
            $nextScoreboard = MatchScoreBoard::updateOrCreate(
                ['match_id' => $scoreboard->match_id, 'innings' => $nextInnings],
                [
                    'team_id' => $nextBattingTeamId,
                    'runs' => 0,
                    'overs' => '0.0',
                    'wickets' => 0,
                    'status' => 'running'
                ]
            );
            $this->updateMatchPlayers($nextScoreboard, $scoreboard);
        }

        return $nextScoreboard;
    }

    public function setInningsStatus(Request $request)
    {
        try {
            if (!$this->canManageScoreboard()) {
                return $this->unauthorizedJson();
            }

            $matchId = $request->match_id ?? $request->route('match_id');
            $match = CricketMatch::findOrFail($matchId);

            $scoreboard = MatchScoreBoard::where('match_id', $matchId)
                ->where('status', 'running')
                ->first();

            if (!$scoreboard) {
                return response()->json([
                    'success' => false,
                    'message' => 'No active innings found.'
                ]);
            }

            $currentInnings = $scoreboard->innings;
            $battingTeamId = $scoreboard->team_id;
            $bowlingTeamId = ($battingTeamId == $match->team_a_id) ? $match->team_b_id : $match->team_a_id;
            $nextBattingTeamId = $bowlingTeamId;
            $nextInnings = $currentInnings + 1;

            // Count wickets fallen
            $currentInningsWicketFallen = FallOfWicket::where('match_id', $matchId)
                ->where('team_id', $battingTeamId)
                ->count();

            // Convert overs string like "4.3" to balls
            list($overs, $balls) = explode('.', $scoreboard->overs);
            $ballsBowled = ($overs * 6) + $balls;
            $maxBalls = $match->max_overs * 6;

            $scoreboards = MatchScoreBoard::where('match_id', $matchId)->get();

            $inningsEnded = false;
            $nextScoreboard = null;

            // Check if innings should end
            if (
                $currentInningsWicketFallen >= $scoreboard->team->players->count() - 1 ||
                $ballsBowled >= $maxBalls
            ) {
                $inningsEnded = true;
                $scoreboard->update(['status' => 'ended']);

                if ($currentInnings < 2) {
                    // Start next innings
                    $nextScoreboard = MatchScoreBoard::updateOrCreate(
                        ['match_id' => $matchId, 'innings' => $nextInnings],
                        [
                            'team_id' => $nextBattingTeamId,
                            'runs' => 0,
                            'overs' => '0.0',
                            'wickets' => 0,
                            'status' => 'running'
                        ]
                    );

                    // Update players status
                    MatchPlayer::where('match_id', $matchId)
                        ->where('team_id', $battingTeamId)
                        ->update(['status' => 'fielding']);

                    MatchPlayer::where('match_id', $matchId)
                        ->where('team_id', $nextBattingTeamId)
                        ->update(['status' => 'ready']);
                } else {
                    // Second innings ended -> decide winner
                    if ($scoreboards->count() == 2) {
                        $firstInnings = $scoreboards->where('innings', 1)->first();
                        $secondInnings = $scoreboards->where('innings', 2)->first();

                        $winner = null;
                        $summary = "";

                        if ($firstInnings->runs > $secondInnings->runs) {
                            $winner = $firstInnings->team;
                            $margin = $firstInnings->runs - $secondInnings->runs;
                            $summary = "{$winner->name} won by {$margin} runs";
                        } elseif ($secondInnings->runs > $firstInnings->runs) {
                            $winner = $secondInnings->team;
                            $remainingWickets = $secondInnings->team->players->count() - 1 - $secondInnings->wickets;
                            $summary = "{$winner->name} won by {$remainingWickets} wickets";
                        } else {
                            $summary = "Match drawn/tied";
                        }

                        $match->update([
                            'status' => 'completed',
                            'winning_team_id' => $winner?->id,
                            'result_summary' => $summary
                        ]);
                    }
                }
            }

            // Prepare message
            $message = "";
            $match->refresh();

            if ($scoreboards->count() == 2 && $match->status === 'completed') {
                $message = $match->result_summary;
            } elseif ($nextScoreboard) {
                $message = "Innings $currentInnings has been completed and Innings $nextInnings has been started.";
            } elseif ($inningsEnded) {
                $message = "Innings $currentInnings has been completed.";
            } else {
                $message = "Innings $currentInnings is still running.";
            }

            event(new CricketMatchUpdateEvent($match, $this->getBroadcastData($match)));

            return response()->json([
                'success' => true,
                'message' => $message,
                'isInningsEnded' => $nextScoreboard != null ? false : $inningsEnded,
                'innings' => $nextInnings,
                'scoreboard' => $nextScoreboard,
                'matchData' => $match
            ]);
        } catch (\Exception $e) {
            Log::error('Error completing innings', [
                'message' => $e->getMessage(),
                'line' => $e->getLine()
            ]);

            return response()->json([
                'success' => false,
                'message' => 'Failed to complete innings.'
            ], 500);
        }
    }

    protected function updateMatchPlayers($currentBoard, $previousBoard)
    {
        try {
            MatchPlayer::where('match_id', $currentBoard->match_id)
                ->where('team_id', $currentBoard->team_id)
                ->update(['status' => 'ready']);

            MatchPlayer::where('match_id', $previousBoard->match_id)
                ->where('team_id', $previousBoard->team_id)
                ->update(['status' => 'fielding']);
        } catch (Exception $e) {
            Log::error("Error Updating Match Player: ", ['data' => $e->getMessage()]);
        }
    }

    public function storeDelivery(Request $request)
    {
        try {
            if (!$this->canManageScoreboard()) {
                return $this->unauthorizedJson();
            }

            $validator = Validator::make($request->all(), [
                'match_id' => 'required|exists:cricket_matches,id',
                'striker_id' => 'nullable|exists:players,id',
                'non_striker_id' => 'nullable|exists:players,id',
                'bowler_id' => 'required|exists:players,id',
                'runs' => 'required|integer|min:0',
                'extras' => 'nullable|array',
                'extras.type' => 'required_with:extras|string',
                'extras.runs' => 'required_with:extras|integer|min:0',
                'extras.run_out' => 'sometimes|boolean',
                'extras.batsman_out' => 'sometimes|exists:players,id',
                'extras.caught_by' => 'sometimes|exists:players,id',
                'extras.stumped_by' => 'sometimes|exists:players,id',
                'caught_by' => 'nullable|exists:players,id',
                'stumped_by' => 'nullable|exists:players,id',
                'fielder_id' => 'nullable|exists:players,id',
                'wicket' => 'nullable|string',
                'batsman_out' => 'nullable|exists:players,id',
                'legal_ball' => 'nullable|boolean',
            ]);

            if ($validator->fails()) {
                Log::error($validator->errors());
                return response()->json([
                    'success' => false,
                    'message' => 'Invalid data',
                    'errors' => $validator->errors()
                ], 422);
            }

            DB::beginTransaction();

            // ---------- Extract request ----------
            $matchId = $request->match_id;
            $strikerId = $request->striker_id;
            $nonStrikerId = $request->non_striker_id;
            $bowlerId = $request->bowler_id;
            $runs = (int) $request->runs;
            $extras = $request->extras ?? [];
            $legalBall = $request->has('legal_ball') ? (bool) $request->legal_ball : true;

            $match = CricketMatch::findOrFail($matchId);
            $maxOvers = $match->max_overs ?? 10;

            // ---------- Find active innings scoreboard ----------
            $scoreboard = MatchScoreBoard::where('match_id', $matchId)
                ->where('status', 'running')
                ->firstOrFail();

            $innings = $scoreboard->innings;

            $lastLegalDelivery = MatchDelivery::where('match_id', $matchId)
                ->where('innings', $scoreboard->innings)
                ->where('delivery_type', 'normal')
                ->latest('id')
                ->first();

            $isOverCompleted = false;

            // ---------- Validate bowler is not same as last over's bowler ----------
            $lastCompletedOver = MatchDelivery::where('match_id', $matchId)
                ->whereNotIn('delivery_type', ['wide', 'no-ball'])
                ->select('over_number')
                ->groupBy('over_number')
                ->havingRaw('COUNT(*) = 6')
                ->orderByDesc('over_number')
                ->first();

            if (
                $lastCompletedOver &&
                $lastCompletedOver->over_number < $maxOvers &&
                (int) $lastCompletedOver->bowler_id === (int) $bowlerId
            ) {
                return response()->json([
                    'success' => false,
                    'message' => 'Current bowler cannot be same as the previous over bowler'
                ]);
            }

            // ---------- Ensure match players exist (if ids provided) ----------
            $striker = MatchPlayer::firstOrCreate(
                ['match_id' => $matchId, 'player_id' => $strikerId, 'team_id' => $scoreboard->team_id],
                ['runs_scored' => 0, 'balls_faced' => 0, 'status' => 'on-strike']
            );

            $nonStriker = MatchPlayer::firstOrCreate(
                ['match_id' => $matchId, 'player_id' => $nonStrikerId, 'team_id' => $scoreboard->team_id],
                ['runs_scored' => 0, 'balls_faced' => 0, 'status' => 'batting']
            );

            $bowlingTeamId = ($scoreboard->team_id == $match->team_a_id) ? $match->team_b_id : $match->team_a_id;

            $bowlerInfo = MatchPlayer::firstOrCreate(
                ['match_id' => $matchId, 'player_id' => $bowlerId, 'team_id' => $bowlingTeamId],
                ['status' => 'bowling']
            );

            // ---------- Delivery type & extras ----------
            $deliveryType = 'normal';
            $extraRuns = 0;
            $batsmanRuns = $runs; // default runs off the bat
            $legalBall = true;
            $extraInputRuns = 0; // initialised here so strike-rotation logic below can safely reference it

            if (!empty($extras)) {
                $type = strtoupper($extras['type'] ?? '');
                $extraInputRuns = (int) ($extras['runs'] ?? 0);

                switch ($type) {
                    case 'NB': // No Ball
                        $deliveryType = 'no-ball';
                        $legalBall = false;
                        $batsmanRuns = $extraInputRuns;
                        $extraRuns = 1;
                        break;

                    case 'WD': // Wide
                        $deliveryType = 'wide';
                        $legalBall = false;
                        $batsmanRuns = 0;
                        $extraRuns = 1 + $extraInputRuns;
                        break;

                    case 'LB': // Leg Bye
                        $deliveryType = 'leg-bye';
                        $batsmanRuns = 0;
                        $extraRuns = $extraInputRuns;
                        break;

                    case 'B': // Bye
                        $deliveryType = 'bye';
                        $batsmanRuns = 0;
                        $extraRuns = $extraInputRuns;
                        break;
                }
            }

            // ---------- Wicket detection ----------
            $wicketType = 'none';
            $wicketStatus = 'none';
            $wicketPlayerId = null;
            $caughtBy = $request->caught_by ?? $request->fielder_id ?? ($extras['caught_by'] ?? null);
            $stumpedBy = $request->stumped_by ?? ($extras['stumped_by'] ?? null);

            if (!empty($request->wicket) && strtolower($request->wicket) !== 'none') {
                [$wicketType, $wicketStatus] = $this->normalizeWicketType($request->wicket);
                $wicketPlayerId = $request->batsman_out ?? $strikerId;
            } elseif (!empty($extras['run_out']) && $extras['run_out']) {
                $wicketType = 'run_out';
                [$wicketType, $wicketStatus] = $this->normalizeWicketType($wicketType);
                $wicketPlayerId = $request->batsman_out ?? ($extras['batsman_out'] ?? null);
            }

            $isWicket = $wicketType !== 'none';

            // ---------- Determine next over/ball (for this innings) ----------
            $lastDelivery = MatchDelivery::where('match_id', $matchId)
                ->where('innings', $innings)
                ->latest('id')
                ->first();

            // default start at over 1 ball 0 -> increment to ball 1 on first legal ball
            $over = $lastDelivery?->over_number ?? 0;
            $ball = $lastDelivery?->ball_in_over ?? 0;

            if ($legalBall) {
                if ($ball == 6) {
                    $over++;
                    $ball = 1;
                    $isOverCompleted = true;
                } else {
                    $ball++;
                    if ($over == 0)
                        $over = 1;
                }
            } else {
                if ($over == 0) {
                    $over = 1;
                    $ball = 0;
                }
            }

            $legalBallsSoFar = MatchDelivery::where('match_id', $matchId)
                ->where('innings', $innings)
                ->whereNotIn('delivery_type', ['no-ball', 'wide'])
                ->count();

            $legalBallsAfterThis = $legalBallsSoFar + ($legalBall ? 1 : 0);
            if ($legalBallsAfterThis > ($maxOvers * 6)) {
                return response()->json([
                    'success' => false,
                    'message' => 'Innings is complete. No more deliveries allowed.'
                ], 400);
            }

            // ---------- Insert delivery ----------
            $deliveryPayload = [
                'match_id' => $matchId,
                'innings' => $innings,
                'over_number' => $over,
                'ball_in_over' => $ball,
                'bowler_id' => $bowlerId,
                'batsman_id' => $strikerId,
                'non_striker_id' => $nonStrikerId,
                'batting_team_id' => $scoreboard->team_id,
                'bowling_team_id' => $bowlerInfo->team_id ?? null,
                'runs_batsman' => $batsmanRuns,
                'runs_extras' => $extraRuns,
                'delivery_type' => $deliveryType,
                'is_wicket' => $isWicket,
                'wicket_type' => $wicketType,
                'wicket_player_id' => $isWicket ? $wicketPlayerId : null,
                'fielder_id' => $caughtBy ?? $stumpedBy ?? null,
            ];

            $delivery = MatchDelivery::create($deliveryPayload);

            // ---------- Update striker & bowler stats ----------
            if ($legalBall)
                $striker->balls_faced += 1;
            if ($batsmanRuns > 0) {
                $striker->runs_scored += $batsmanRuns;
            }

            if ($runs == 4) {
                $striker->fours += 1;
            } elseif ($runs == 6) {
                $striker->sixes += 1;
            }

            $striker->save();

            $oversBowled = $bowlerInfo->overs_bowled ?? 0;
            $overPart = floor($oversBowled);
            $ballPart = round(($oversBowled - $overPart) * 10);

            if (!in_array($deliveryType, ['no-ball', 'wide'])) {
                $ballPart++;
                if ($ballPart >= 6) {
                    $overPart++;
                    $ballPart = 0;
                }
            }

            $bowlerInfo->overs_bowled = $overPart + ($ballPart / 10);
            $bowlerInfo->runs_conceded = ($bowlerInfo->runs_conceded ?? 0) + ($batsmanRuns + $extraRuns);
            if ($isWicket && $wicketType != 'run_out') {
                $bowlerInfo->wickets_taken = ($bowlerInfo->wickets_taken ?? 0) + 1;
            }
            $bowlerInfo->save();

            // ---------- Update scoreboard totals ----------
            $totalRunsValid = $batsmanRuns + $extraRuns;
            $totalRuns = MatchDelivery::where('match_id', $matchId)
                ->where('innings', $innings)
                ->sum(DB::raw('runs_batsman + runs_extras'));

            $legalBalls = MatchDelivery::where('match_id', $matchId)
                ->where('innings', $innings)
                ->whereNotIn('delivery_type', ['no-ball', 'wide'])
                ->count();

            $oversFormatted = intval(intdiv($legalBalls, 6)) . '.' . ($legalBalls % 6);

            $scoreboard->update([
                'runs' => $totalRuns,
                'overs' => $oversFormatted,
                'status' => $scoreboard->status
            ]);

            // ---------- Partnership logic (update player1/player2 runs correctly) ----------
            $partnership = Partnership::where('match_id', $matchId)
                ->where('team_id', $scoreboard->team_id)
                ->whereNull('wicket_id')
                ->latest('id')
                ->first();

            if ($partnership) {
                $partnership->runs += $totalRunsValid;
                $partnership->balls += $legalBall ? 1 : 0;
                $partnership->player1_runs += ($partnership->batter_1_id == $strikerId) ? $totalRunsValid : 0;
                $partnership->player2_runs += ($partnership->batter_2_id == $strikerId) ? $totalRunsValid : 0;
                $partnership->save();
            } else {
                Partnership::create([
                    'match_id' => $matchId,
                    'team_id' => $scoreboard->team_id,
                    'batter_1_id' => $strikerId,
                    'player1_runs' => ($striker->runs_scored ?? 0),
                    'batter_2_id' => $nonStrikerId,
                    'player2_runs' => ($nonStriker->runs_scored ?? 0),
                    'runs' => $runs,
                    'balls' => $legalBall ? 1 : 0,
                    'start_over' => $over,
                    'end_over' => $over,
                ]);
            }

            // ---------- Handle wicket & FallOfWicket ----------
            if ($isWicket && $wicketPlayerId) {
                $scoreboard->increment('wickets');

                $fow = FallOfWicket::create([
                    'match_id' => $matchId,
                    'team_id' => $scoreboard->team_id,
                    'wicket_number' => $scoreboard->wickets,
                    'runs' => $scoreboard->runs,
                    'overs' => $scoreboard->overs,
                    'batter_id' => $wicketPlayerId,
                    'bowler_id' => $bowlerId,
                    'dismissal_type' => $wicketType,
                    'fielder_id' => $caughtBy ?? $stumpedBy ?? $request->fielder_id,
                ]);

                // update dismissed player status
                if ($wicketPlayerId == $strikerId) {
                    $striker->status = $wicketStatus;
                    $striker->save();
                } elseif ($wicketPlayerId == $nonStrikerId) {
                    $nonStriker->status = $wicketStatus;
                    $nonStriker->save();
                }

                if ($partnership) {
                    $partnership->wicket_id = $fow->id;
                    $partnership->save();
                }
            }

            // ---------- Strike rotation ----------
            if ($legalBall) {
                // End of over
                if ($ball == 6) {
                    $this->doSwitchStrike($matchId);
                }

                // Odd runs: switch strike immediately via DB
                if ($batsmanRuns % 2 !== 0) {
                    $this->doSwitchStrike($matchId);
                }
            } elseif ($deliveryType === 'no-ball' || $deliveryType === 'wide') {
                $extraRunCount = $deliveryType === 'wide' ? $extraInputRuns : $batsmanRuns;
                if ($extraRunCount % 2 !== 0) {
                    $this->doSwitchStrike($matchId);
                }
            }

            // ---------- Final ball of last over => end innings & create next scoreboard ----------
            $wasLastLegalBallOfInnings = $legalBall && ($over == $maxOvers) && ($ball == 6);
            $isInningsEnded = false;

            if ($wasLastLegalBallOfInnings || ($isWicket && $scoreboard->wickets >= 10)) {
                $this->switchTeam($match, $scoreboard);
                $isInningsEnded = true;

                $completedInnings = MatchScoreBoard::where('match_id', $matchId)
                    ->whereIn('status', ['ended'])
                    ->count();

                if ($completedInnings >= 2) {
                    $innings1 = MatchScoreBoard::where('match_id', $matchId)->where('innings', 1)->first();
                    $innings2 = MatchScoreBoard::where('match_id', $matchId)->where('innings', 2)->first();

                    if ($innings1 && $innings2) {
                        $innings2TeamName = optional($innings2->team)->name ?? 'Team 2';
                        $innings1TeamName = optional($innings1->team)->name ?? 'Team 1';

                        if ($innings2->runs > $innings1->runs) {
                            $remainingWickets = 10 - $innings2->wickets;
                            $match->winning_team_id = $innings2->team_id;
                            $match->result_summary = "{$innings2TeamName} won by {$remainingWickets} wickets";
                        } elseif ($innings2->runs < $innings1->runs && $innings2->overs >= $maxOvers) {
                            $margin = $innings1->runs - $innings2->runs;
                            $match->winning_team_id = $innings1->team_id;
                            $match->result_summary = "{$innings1TeamName} won by {$margin} runs";
                        } elseif ($innings2->runs == $innings1->runs) {
                            $match->winning_team_id = null;
                            $match->result_summary = "Match tied";
                        }

                        $match->status = 'completed';
                        $match->save();
                    }
                }
            }

            // ---------- If innings 1 just completed overs, ensure innings 2 is activated ----------
            if ($innings == 1 && intdiv($legalBalls, 6) >= $maxOvers) {
                $innings2 = MatchScoreBoard::where('match_id', $matchId)->where('innings', 2)->first();
                if ($innings2 && $innings2->status !== 'running') {
                    $innings2->status = 'running';
                    $innings2->save();
                }
                $isInningsEnded = true;
            }

            if ($innings == 2) {
                $innings1 = MatchScoreBoard::where('match_id', $matchId)->where('innings', 1)->first();
                $target = $innings1->runs + 1;

                if ($scoreboard->runs >= $target) {
                    $remainingWickets = $scoreboard->team->players->count() - $scoreboard->wickets;

                    $match->winning_team_id = $scoreboard->team_id;
                    $match->result_summary = $scoreboard->team->name . " won by {$remainingWickets} wickets";
                    $match->status = 'completed';
                    $match->save();

                    $scoreboard->status = 'ended';
                    $scoreboard->save();

                    $isInningsEnded = true;
                }
            }

            // ---------- Update Player Statistics ----------
            if ($strikerId) {
                $strikerStat = PlayerStat::firstOrCreate(['player_id' => $strikerId]);

                $strikerStat->total_runs = ($strikerStat->total_runs ?? 0) + $batsmanRuns;
                $strikerStat->balls_faced = ($strikerStat->balls_faced ?? 0) + ($legalBall ? 1 : 0);
                $strikerStat->fours = ($strikerStat->fours ?? 0) + (($batsmanRuns == 4) ? 1 : 0);
                $strikerStat->sixes = ($strikerStat->sixes ?? 0) + (($batsmanRuns == 6) ? 1 : 0);

                if ($strikerStat->balls_faced > 0) {
                    $strikerStat->strike_rate = round(($strikerStat->total_runs / $strikerStat->balls_faced) * 100, 2);
                }

                $strikerStat->save();

                if ($match->tournament) {
                    $playerTournamentStat = TournamentPlayerStat::firstOrCreate([
                        'tournament_id' => $match->tournament_id,
                        'player_id' => $strikerId
                    ]);

                    if (!$playerTournamentStat->matches_played || $playerTournamentStat->last_match_id != $match->id) {
                        $playerTournamentStat->matches_played += 1;
                        $playerTournamentStat->last_match_id = $match->id;
                    }

                    if ($legalBall && $runs >= 0) {
                        if (!$playerTournamentStat->innings_batted || $playerTournamentStat->last_batting_match_id != $match->id) {
                            $playerTournamentStat->innings_batted += 1;
                            $playerTournamentStat->last_batting_match_id = $match->id;
                        }
                    }

                    $playerTournamentStat->total_runs = ($playerTournamentStat->total_runs ?? 0) + $batsmanRuns;
                    $playerTournamentStat->balls_faced = ($playerTournamentStat->balls_faced ?? 0) + ($legalBall ? 1 : 0);
                    $playerTournamentStat->fours = ($playerTournamentStat->fours ?? 0) + (($batsmanRuns == 4) ? 1 : 0);
                    $playerTournamentStat->sixes = ($playerTournamentStat->sixes ?? 0) + (($batsmanRuns == 6) ? 1 : 0);

                    if ($playerTournamentStat->balls_faced > 0) {
                        $playerTournamentStat->strike_rate = round(($playerTournamentStat->total_runs / $playerTournamentStat->balls_faced) * 100, 2);
                    }

                    if ($playerTournamentStat->total_runs >= 50 && $playerTournamentStat->total_runs < 100) {
                        $playerTournamentStat->fifties = ($playerTournamentStat->fifties ?? 0) + 1;
                    } elseif ($playerTournamentStat->total_runs >= 100) {
                        $playerTournamentStat->hundreds = ($playerTournamentStat->hundreds ?? 0) + 1;
                    }

                    if ($playerTournamentStat->innings_batted > 0) {
                        $playerTournamentStat->average = round($playerTournamentStat->total_runs / $playerTournamentStat->innings_batted, 2);
                    }

                    $playerTournamentStat->save();
                }
            }

            if ($nonStrikerId) {
                $nonStrikerStat = PlayerStat::firstOrCreate(['player_id' => $nonStrikerId]);
                $nonStrikerStat->save();

                if ($match->tournament) {
                    $playerTournamentStat = TournamentPlayerStat::firstOrCreate([
                        'tournament_id' => $match->tournament_id,
                        'player_id' => $nonStrikerId
                    ]);

                    if (!$playerTournamentStat->matches_played || $playerTournamentStat->last_match_id != $match->id) {
                        $playerTournamentStat->matches_played += 1;
                        $playerTournamentStat->last_match_id = $match->id;
                    }

                    if ($legalBall && $runs >= 0) {
                        if (!$playerTournamentStat->innings_batted || $playerTournamentStat->last_batting_match_id != $match->id) {
                            $playerTournamentStat->innings_batted += 1;
                            $playerTournamentStat->last_batting_match_id = $match->id;
                        }
                    }

                    $playerTournamentStat->total_runs = ($playerTournamentStat->total_runs ?? 0) + $runs;
                    $playerTournamentStat->balls_faced = ($playerTournamentStat->balls_faced ?? 0) + ($legalBall ? 1 : 0);
                    $playerTournamentStat->fours = ($playerTournamentStat->fours ?? 0) + (($runs == 4) ? 1 : 0);
                    $playerTournamentStat->sixes = ($playerTournamentStat->sixes ?? 0) + (($runs == 6) ? 1 : 0);

                    if ($playerTournamentStat->balls_faced > 0) {
                        $playerTournamentStat->strike_rate = round(($playerTournamentStat->total_runs / $playerTournamentStat->balls_faced) * 100, 2);
                    }

                    if ($playerTournamentStat->total_runs >= 50 && $playerTournamentStat->total_runs < 100) {
                        $playerTournamentStat->fifties += 1;
                    } elseif ($playerTournamentStat->total_runs >= 100) {
                        $playerTournamentStat->hundreds += 1;
                    }

                    if ($playerTournamentStat->innings_batted > 0) {
                        $playerTournamentStat->average = round($playerTournamentStat->total_runs / $playerTournamentStat->innings_batted, 2);
                    }

                    $playerTournamentStat->save();
                }
            }

            if ($bowlerId) {
                $bowlerStat = PlayerStat::firstOrCreate(['player_id' => $bowlerId]);
                if ($legalBall && ($bowlerStat->overs_bowled ?? 0) == 0) {
                    $bowlerStat->innings_bowled = ($bowlerStat->innings_bowled ?? 0) + 1;
                }

                $bowlerStat->overs_bowled = ($bowlerStat->overs_bowled ?? 0) + ($legalBall ? 1 : 0);
                $bowlerStat->runs_conceded = ($bowlerStat->runs_conceded ?? 0) + $totalRunsValid;
                if ($isWicket && $wicketType != 'run_out') {
                    $bowlerStat->wickets = ($bowlerStat->wickets ?? 0) + 1;
                }

                // derived bowling metrics
                if ($bowlerStat->overs_bowled > 0) {
                    $oversAsFloat = $bowlerStat->overs_bowled / 6; // convert balls back to overs
                    $bowlerStat->economy_rate = round($bowlerStat->runs_conceded / $oversAsFloat, 2);

                    if ($bowlerStat->wickets > 0) {
                        $bowlerStat->bowling_average = round($bowlerStat->runs_conceded / $bowlerStat->wickets, 2);
                    }
                }

                $bowlerStat->save();

                if ($match->tournament) {
                    $playerTournamentStat = TournamentPlayerStat::firstOrCreate([
                        'tournament_id' => $match->tournament_id,
                        'player_id' => $bowlerId
                    ]);

                    if (!$playerTournamentStat->matches_played || $playerTournamentStat->last_match_id != $match->id) {
                        $playerTournamentStat->matches_played += 1;
                        $playerTournamentStat->last_match_id = $match->id;
                    }

                    if ($legalBall && (!$playerTournamentStat->last_bowling_match_id || $playerTournamentStat->last_bowling_match_id != $match->id)) {
                        $playerTournamentStat->innings_bowled = ($playerTournamentStat->innings_bowled ?? 0) + 1;
                        $playerTournamentStat->last_bowling_match_id = $match->id;
                    }

                    if (!in_array($deliveryType, ['no-ball', 'wide'])) {
                        $playerTournamentStat->overs_bowled = ($playerTournamentStat->overs_bowled ?? 0) + 1; // count only legal balls
                    }

                    $playerTournamentStat->runs_conceded = ($playerTournamentStat->runs_conceded ?? 0) + $runs + (in_array($deliveryType, ['no-ball', 'wide']) ? 1 : 0);

                    if ($isWicket && $wicketType != 'run_out') {
                        $playerTournamentStat->wickets = ($playerTournamentStat->wickets ?? 0) + 1;
                    }

                    if ($playerTournamentStat->overs_bowled > 0) {
                        $oversAsFloat = $playerTournamentStat->overs_bowled / 6; // balls → overs
                        $playerTournamentStat->economy_rate = round($playerTournamentStat->runs_conceded / $oversAsFloat, 2);
                    }

                    if ($playerTournamentStat->wickets > 0) {
                        $playerTournamentStat->bowling_average = round($playerTournamentStat->runs_conceded / $playerTournamentStat->wickets, 2);
                    }

                    $playerTournamentStat->save();
                }
            }

            DB::commit();

            $data = json_decode($this->getCurrentOver($matchId)->getContent(), true);
            Log::info('Current Over Data: ', $data);
            $isOverCompleted = $data['legalBalls'] >= 6;

            // ---------- Broadcast real-time update ----------
            $broadcastData = $this->getBroadcastData($match);
            event(new CricketMatchUpdateEvent($match, $broadcastData));

            // ---------- Prepare response ----------
            return response()->json([
                'success' => true,
                'message' => 'Delivery recorded successfully',
                'isInningsEnded' => (bool) $isInningsEnded,
                'isOverEnded' => (bool) $isOverCompleted,
                'updated_state' => [
                    'striker' => in_array($striker->status, ['batting', 'on-strike']) ? [
                        'id' => $striker->player_id,
                        'name' => $striker->player?->user?->full_name ?? '---',
                        'runs' => $striker->runs_scored,
                        'balls' => $striker->balls_faced,
                        'status' => $striker->status,
                    ] : null,
                    'nonStriker' => in_array($nonStriker->status, ['batting', 'on-strike']) ? [
                        'id' => $nonStriker->player_id,
                        'name' => $nonStriker->player?->user?->full_name ?? '---',
                        'runs' => $nonStriker->runs_scored,
                        'balls' => $nonStriker->balls_faced,
                        'status' => $nonStriker->status,
                    ] : null,
                    'team' => [
                        'score' => $scoreboard->runs,
                        'overs' => $scoreboard->overs,
                        'wickets' => $scoreboard->wickets,
                        'status' => $scoreboard->status,
                    ]
                ]
            ]);
        } catch (\Exception $e) {
            DB::rollBack();
            Log::error('Error storing delivery', [
                'message' => $e->getMessage(),
                'line' => $e->getLine()
            ]);

            return response()->json([
                'success' => false,
                'message' => 'Failed to record delivery'
            ], 500);
        }
    }

    public function undoLastDelivery(Request $request)
    {
        try {
            if (!$this->canManageScoreboard()) {
                return $this->unauthorizedJson();
            }

            $validator = Validator::make($request->all(), [
                'match_id' => 'required|exists:cricket_matches,id',
            ]);

            if ($validator->fails()) {
                return response()->json([
                    'success' => false,
                    'message' => 'Invalid data',
                    'errors' => $validator->errors()
                ], 422);
            }

            $matchId = $request->match_id;

            // Get the last delivery
            $lastDelivery = MatchDelivery::where('match_id', $matchId)
                ->latest('id')
                ->first();

            if (!$lastDelivery) {
                return response()->json([
                    'success' => false,
                    'message' => 'No deliveries to undo.'
                ], 400);
            }

            DB::beginTransaction();

            // Get related models
            $scoreboard = MatchScoreBoard::where('match_id', $matchId)
                ->where('status', 'running')
                ->first();

            if (!$scoreboard) {
                throw new \Exception('Scoreboard not found');
            }

            $striker = MatchPlayer::where('match_id', $matchId)
                ->where('player_id', $lastDelivery->batsman_id)
                ->first();
            $nonStriker = MatchPlayer::where('match_id', $matchId)
                ->where('player_id', $lastDelivery->non_striker_id)
                ->first();
            $bowler = MatchPlayer::where('match_id', $matchId)
                ->where('player_id', $lastDelivery->bowler_id)
                ->first();

            if (!$striker || !$nonStriker || !$bowler) {
                throw new \Exception('Players involved in last delivery not found');
            }

            // Check if this was the 6th ball of the over
            $wasCompleteOver = false;
            $overNumber = $lastDelivery->over_number;

            // Count legal balls in this over BEFORE deletion
            $legalBallsInOver = MatchDelivery::where('match_id', $matchId)
                ->where('innings', $lastDelivery->innings)
                ->where('over_number', $overNumber)
                ->whereNotIn('delivery_type', ['wide', 'no-ball'])
                ->count();

            $wasCompleteOver = ($legalBallsInOver >= 6);

            // Store bowler team ID for status update
            $bowlerTeamId = $bowler->team_id;

            // -------------------------------
            // 1. Update MatchScoreboard
            // -------------------------------
            $scoreboard->runs -= ($lastDelivery->runs_batsman + $lastDelivery->runs_extras);
            $scoreboard->extras = $scoreboard->extras > 0 ? $scoreboard->extras - $lastDelivery->runs_extras : 0;
            if ($lastDelivery->is_wicket) {
                $scoreboard->wickets -= 1;
            }

            if (in_array($lastDelivery->delivery_type, ['normal', 'bye', 'leg-bye'])) {
                [$overs, $balls] = array_pad(
                    explode('.', number_format((float) $scoreboard->overs, 1, '.', '')),
                    2,
                    0
                );

                $totalBalls = ($overs * 6) + $balls;
                $totalBalls = max(0, $totalBalls - 1);
                $scoreboard->overs = floor($totalBalls / 6) . '.' . ($totalBalls % 6);
            }

            $scoreboard->update();

            // -------------------------------
            // 2. Update MatchPlayer stats
            // -------------------------------
            $striker->runs_scored -= $lastDelivery->runs_batsman;
            if (!in_array($lastDelivery->delivery_type, ['wide', 'no-ball'])) {
                $striker->balls_faced -= 1;
            }
            if ($lastDelivery->is_wicket) {
                $striker->status = 'batting';
                $striker->out_type = null;
            }
            $striker->save();

            // Bowler stats
            [$overs, $balls] = array_pad(
                explode('.', (string) $bowler->overs_bowled),
                2,
                0
            );

            // Convert to total balls
            $totalBalls = ($overs * 6) + $balls;

            // Undo only legal delivery
            if (!in_array($lastDelivery->delivery_type, ['wide', 'no-ball'])) {
                $totalBalls = max(0, $totalBalls - 1);
            }

            // Convert back to overs.balls
            $bowler->overs_bowled = floor($totalBalls / 6) . '.' . ($totalBalls % 6);

            $bowler->runs_conceded -= ($lastDelivery->runs_batsman + $lastDelivery->runs_extras);

            if ($lastDelivery->is_wicket) {
                $bowler->wickets_taken -= 1;
            }

            // Handle bowler status if over is no longer complete
            if ($wasCompleteOver && $legalBallsInOver <= 6) {
                // This was the 6th ball of a complete over, now it's incomplete
                // Set the bowler's status back to 'bowling'
                $bowler->status = 'bowling';
            }

            $bowler->save();

            // -------------------------------
            // 3. Update other fielding players' status if over became incomplete
            // -------------------------------
            if ($wasCompleteOver && $legalBallsInOver <= 6) {
                // Get all players from the bowling team
                $bowlingTeamPlayers = MatchPlayer::where('match_id', $matchId)
                    ->where('team_id', $bowlerTeamId)
                    ->get();

                foreach ($bowlingTeamPlayers as $player) {
                    if ($player->player_id == $bowler->player_id) {
                        // Already set to 'bowling'
                        continue;
                    }

                    // Check if player was involved in this over
                    $wasInvolvedInOver = MatchDelivery::where('match_id', $matchId)
                        ->where('innings', $lastDelivery->innings)
                        ->where('over_number', $overNumber)
                        ->where(function ($query) use ($player) {
                            $query->where('bowler_id', $player->player_id)
                                ->orWhere('fielder_id', $player->player_id);
                        })
                        ->exists();

                    if ($wasInvolvedInOver || $player->status == 'fielding') {
                        // Keep as fielding if they were involved or already fielding
                        $player->status = 'fielding';
                    } else {
                        // Set to default status
                        $player->status = null;
                    }

                    $player->save();
                }

                // Also need to switch strike back if this was the 6th ball
                // Get the two active batters
                $activeBatters = MatchPlayer::where('match_id', $matchId)
                    ->where('team_id', $scoreboard->team_id)
                    ->whereIn('status', ['on-strike', 'batting'])
                    ->orderByDesc('created_at')
                    ->take(2)
                    ->get();

                if ($activeBatters->count() == 2) {
                    // Switch strike back (reverse the end-of-over switch)
                    $currentStriker = $activeBatters->firstWhere('status', 'on-strike');
                    $currentNonStriker = $activeBatters->firstWhere('status', 'batting');

                    if ($currentStriker && $currentNonStriker) {
                        // Swap their statuses
                        $currentStriker->status = 'batting';
                        $currentNonStriker->status = 'on-strike';

                        $currentStriker->save();
                        $currentNonStriker->save();
                    }
                }
            }

            // -------------------------------
            // 4. Update PlayerStat (overall player stats)
            // -------------------------------
            $playerStat = PlayerStat::firstOrCreate(['player_id' => $striker->player_id]);
            $playerStat->total_runs -= $lastDelivery->runs_batsman;
            if (!in_array($lastDelivery->delivery_type, ['wide', 'no-ball'])) {
                $playerStat->balls_faced -= 1;
            }
            if ($lastDelivery->is_wicket) {
                $playerStat->dismissals -= 1;
            }
            $playerStat->save();

            // Bowler stats
            $bowlerStat = PlayerStat::where('player_id', $bowler->player_id)->first();
            if ($bowlerStat) {
                if (!in_array($lastDelivery->delivery_type, ['wide', 'no-ball'])) {
                    $bowlerStat->overs_bowled -= 1; // overs_bowled = total balls
                }

                $bowlerStat->runs_conceded -= ($lastDelivery->runs_batsman + $lastDelivery->runs_extras);

                if ($lastDelivery->is_wicket) {
                    $bowlerStat->wickets -= 1;
                }

                $bowlerStat->save();
            }

            // -------------------------------
            // 5. Update TournamentPlayerStat if match is tournament
            // -------------------------------
            $match = CricketMatch::find($matchId);
            if ($match && $match->tournament_id) {
                $tournamentPlayerStat = TournamentPlayerStat::firstOrCreate([
                    'player_id' => $striker->player_id,
                    'tournament_id' => $match->tournament_id
                ]);
                $tournamentPlayerStat->runs -= $lastDelivery->runs_batsman;
                if ($lastDelivery->is_wicket) {
                    $tournamentPlayerStat->dismissals -= 1;
                }
                $tournamentPlayerStat->save();

                $tournamentBowlerStat = TournamentPlayerStat::firstOrCreate([
                    'player_id' => $bowler->player_id,
                    'tournament_id' => $match->tournament_id
                ]);

                // Handle overs_bowled conversion
                [$overs, $balls] = explode('.', (string) ($tournamentBowlerStat->overs_bowled ?? '0.0'));
                $totalBalls = ($overs * 6) + $balls;

                if (!in_array($lastDelivery->delivery_type, ['wide', 'no-ball'])) {
                    $totalBalls -= 1;
                }

                $tournamentBowlerStat->overs_bowled = floor($totalBalls / 6) . '.' . ($totalBalls % 6);
                $tournamentBowlerStat->runs_conceded -= $lastDelivery->runs_batsman + $lastDelivery->runs_extras;

                if ($lastDelivery->is_wicket) {
                    $tournamentBowlerStat->wickets -= 1;
                }

                $tournamentBowlerStat->save();
            }

            // -------------------------------
            // 6. Update partnerships and fall of wickets if needed
            // -------------------------------
            if ($lastDelivery->is_wicket) {
                // Remove the fall of wicket record
                $fallOfWicket = FallOfWicket::where('match_id', $matchId)
                    ->where('batter_id', $lastDelivery->wicket_player_id)
                    ->latest('id')
                    ->first();

                if ($fallOfWicket) {
                    // Update the partnership that ended with this wicket
                    $partnership = Partnership::where('wicket_id', $fallOfWicket->id)
                        ->first();

                    if ($partnership) {
                        $partnership->wicket_id = null;
                        $partnership->save();
                    }

                    $fallOfWicket->delete();
                }
            }

            // Update partnership runs if needed
            $partnership = Partnership::where('match_id', $matchId)
                ->where('team_id', $scoreboard->team_id)
                ->whereNull('wicket_id')
                ->latest('id')
                ->first();

            if ($partnership) {
                $partnership->runs -= ($lastDelivery->runs_batsman + $lastDelivery->runs_extras);
                if (!in_array($lastDelivery->delivery_type, ['wide', 'no-ball'])) {
                    $partnership->balls -= 1;
                }

                // Update individual batter runs
                if ($partnership->batter_1_id == $striker->player_id) {
                    $partnership->player1_runs -= $lastDelivery->runs_batsman;
                } elseif ($partnership->batter_2_id == $striker->player_id) {
                    $partnership->player2_runs -= $lastDelivery->runs_batsman;
                }

                $partnership->save();
            }

            // -------------------------------
            // 7. Delete the delivery
            // -------------------------------
            $lastDelivery->delete();

            DB::commit();

            return response()->json([
                'success' => true,
                'message' => 'Last delivery undone successfully.',
                'data' => [
                    'was_complete_over' => $wasCompleteOver,
                    'over_number' => $overNumber,
                    'legal_balls_remaining' => $legalBallsInOver - 1,
                    'bowler_status_updated' => $wasCompleteOver ? 'bowling' : 'unchanged'
                ]
            ]);
        } catch (\Exception $e) {
            DB::rollBack();
            Log::error('Error undoing last delivery', [
                'message' => $e->getMessage(),
                'line' => $e->getLine()
            ]);

            return response()->json([
                'success' => false,
                'message' => 'Failed to undo last delivery.'
            ], 500);
        }
    }

    public function destroy($id)
    {
        try {
            if (!$this->canAny([$this->module . '-delete'])) {
                throw new Exception('Unauthorized Access');
            }

            $match = CricketMatch::with(['scoreboard', 'players', 'deliveries'])->findOrFail($id);

            if ($match->status === 'completed') {
                return redirect()->back()->with('error', 'Completed matches cannot be deleted.');
            }

            DB::beginTransaction();

            if ($match->deliveries()->exists()) {
                $match->deliveries()->delete();
            }

            if ($match->scoreboard()->exists()) {
                $match->scoreboard()->delete();
            }

            // if ($match->players()->exists()) {
            //     $match->players()->detach();
            // }

            $match->delete();

            DB::commit();

            return redirect()->back()->with([
                'success' => true,
                'message' => 'Match and related data deleted successfully.'
            ]);
        } catch (\Exception $e) {
            DB::rollBack();

            Log::error('Error deleting match: ', [
                'message' => $e->getMessage(),
                'file' => $e->getFile(),
                'line' => $e->getLine(),
            ]);

            return redirect()->back()->with('error', 'Something went wrong while deleting the match.');
        }
    }

    private function getBroadcastData(CricketMatch $match)
    {
        $matchId = $match->id;

        $scoreboard = MatchScoreBoard::where('match_id', $matchId)
            ->where('status', 'running')
            ->first();

        if (!$scoreboard) {
            $scoreboard = MatchScoreBoard::where('match_id', $matchId)
                ->orderBy('innings', 'desc')
                ->first();
        }

        $battingTeamId = $scoreboard ? $scoreboard->team_id : null;
        $bowlingTeamId = ($battingTeamId == $match->team_a_id) ? $match->team_b_id : $match->team_a_id;

        $striker = MatchPlayer::where('match_id', $matchId)
            ->where('team_id', $battingTeamId)
            ->whereIn('status', ['on-strike', 'batting'])
            ->orderByRaw("FIELD(status, 'on-strike', 'batting')")
            ->first();

        $nonStriker = MatchPlayer::where('match_id', $matchId)
            ->where('team_id', $battingTeamId)
            ->whereIn('status', ['on-strike', 'batting'])
            ->where('player_id', '!=', $striker ? $striker->player_id : null)
            ->first();

        $currentBowler = MatchPlayer::where('match_id', $matchId)
            ->where('team_id', $bowlingTeamId)
            ->where('status', 'bowling')
            ->first();

        // Get over data
        $overData = [];
        try {
            $overResponse = $this->getCurrentOver($matchId);
            $overContent = json_decode($overResponse->getContent(), true);
            $overData = $overContent['balls'] ?? [];
        } catch (\Exception $e) {
            Log::error('Error getting over data for broadcast: ' . $e->getMessage());
        }

        return [
            'battingTeam' => ['id' => $battingTeamId],
            'bowlingTeam' => ['id' => $bowlingTeamId],
            'scoreboard' => $scoreboard ? [
                'team_id' => $scoreboard->team_id,
                'innings' => $scoreboard->innings,
                'runs' => $scoreboard->runs,
                'wickets' => $scoreboard->wickets,
                'overs' => $scoreboard->overs,
                'status' => $scoreboard->status,
                'projected_score' => $this->calculateProjected($scoreboard, $match->max_overs)
            ] : null,
            'striker' => $striker ? [
                'id' => $striker->player_id,
                'name' => $striker->player?->user?->full_name ?? '---',
                'runs' => $striker->runs_scored,
                'balls' => $striker->balls_faced,
                'status' => $striker->status,
                'image' => $striker->player?->image,
            ] : null,
            'nonStriker' => $nonStriker ? [
                'id' => $nonStriker->player_id,
                'name' => $nonStriker->player?->user?->full_name ?? '---',
                'runs' => $nonStriker->runs_scored,
                'balls' => $nonStriker->balls_faced,
                'status' => $nonStriker->status,
                'image' => $nonStriker->player?->image,
            ] : null,
            'currentBowler' => $currentBowler ? [
                'id' => $currentBowler->player_id,
                'name' => $currentBowler->player?->user?->full_name ?? '---',
                'overs' => $currentBowler->overs_bowled,
                'runs' => $currentBowler->runs_conceded,
                'wickets' => $currentBowler->wickets_taken,
                'image' => $currentBowler->player?->image,
            ] : null,
            'currentOver' => [
                'over_number' => $scoreboard ? floor(floatval($scoreboard->overs)) + 1 : 1,
                'balls' => $overData
            ],
            'inningsStatus' => $scoreboard ? $scoreboard->status : $match->status,
            'matchResult' => $match->result_summary,
            'is_tournament' => (bool) $match->tournament_id,
            'yetToBatList' => $battingTeamId ? $this->yetToBatPlayers($battingTeamId, $matchId) : [],
        ];
    }

    private function yetToBatPlayers($teamId, $matchId)
    {
        $allPlayers = Player::whereHas('teams', function ($q) use ($teamId) {
            $q->where('teams.id', $teamId);
        })->with('user')->get();

        $battedPlayerIds = MatchPlayer::where('match_id', $matchId)
            ->where('team_id', $teamId)
            ->pluck('player_id')
            ->toArray();

        return $allPlayers->filter(function ($player) use ($battedPlayerIds) {
            return !in_array($player->id, $battedPlayerIds);
        })->map(function ($player) {
            return [
                'id' => $player->id,
                'name' => $player->user?->full_name ?? '---',
                'role' => $player->player_role,
                'image' => $player->image,
                'player' => $player // Nesting for frontend compatibility
            ];
        })->values();
    }

    private function normalizeWicketType($type)
    {
        $key = strtolower(str_replace(['-', '_'], ' ', (string) $type));

        switch ($key) {
            case 'run out':
                return ['run out', 'run_out'];
            case 'hit wicket':
                return ['hit wicket', 'hit-wicket'];
            case 'retired hurt':
                return ['retired hurt', 'retired-hurt'];
            case 'stumped':
                return ['stumped', 'stumped'];
            case 'caught':
                return ['caught', 'caught'];
            case 'lbw':
                return ['lbw', 'lbw'];
            case 'bowled':
                return ['bowled', 'bowled'];
            default:
                return ['none', 'none'];
        }
    }

    private function calculateProjected($scoreboard, $maxOvers)
    {
        if (!$scoreboard || !$scoreboard->overs || $maxOvers <= 0)
            return 0;

        $oversParts = explode('.', (string) $scoreboard->overs);
        $overs = (int) $oversParts[0];
        $balls = (int) ($oversParts[1] ?? 0);
        $totalBalls = ($overs * 6) + $balls;

        if ($totalBalls === 0)
            return 0;

        $runs = $scoreboard->runs;
        $runRate = ($runs / $totalBalls) * 6;

        return (int) round($runRate * $maxOvers);
    }
}
